...
class Wechat
{
    ...
    protected function resetflag($weixin_name, $from)
    {
		...
		$data = $db->query($sql_flag);
       // $result = mysqli_query($this->dblink, $sql_flag);
//error_log($weixin_name.$xieverify);		
    ...
    {
        //mysqli_query($this->dblink, "UPDATE  `weixin_flag` SET `flag` = '0'  WHERE  `openid`='$from'");
		$db = mysql::getInstance();
		$data = $db->query("UPDATE  `weixin_flag` SET `flag` = '0'  WHERE  `openid`='$from'");
    }
     ...
     * @return void
     */
    protected function sqlselect($sql_name, $from, $type = '', $which = '',$query_type = MYSQL_NUM)
    {
		...
		$db = mysql::getInstance();
        error_log($sql_check);
        //$query_check = mysqli_query($this->dblink,$sql_check);
        if ($type == "row") {
            //$q = mysqli_fetch_row($sql_check);
			$q = $db->getRow($sql_check,$query_type);
        } else {
            //$q = mysqli_fetch_array($sql_check);
			$q = $db->getRows($sql_check,$query_type);
        }
    ...
    {
        $reply;
         $q = $this->sqlselect('flag', $from,"row","",MYSQL_ASSOC);
        if ($q['nickname'] != '') {
    ...
    {
		error_log($this->getRequest('fromusername'));
		error_log($this->getRequest('tousername'));
		error_log($items."item");
		error_log($funcFlag."funcflag");
    ...
    public function run()
    {
        ...
        $check = $this->sqlselect('flag', $from, "row","",MYSQL_NUM);
        //error_log($xuanzezu[18]);
        //error_log($this->getRequest('content'));
        //error_log($xuanzezu[19]);
		//error_log($check[1]."2:".$check[2]);
	   
        $checkflag = isset($check[2]);


		if($this->getRequest('msgtype') == 'event') {
        ...
        if ($xuanzezu[18] && $this->getRequest('content') != $xuanzezu[19] && ! $check[2]) {
            ...
            error_log("测试3下！");
			exit();
            exit($this->sendxml(file_get_contents("php://input"), $xuanzezu[20], $xuanzezu[21]));
        } else {
            if (! $check[2]) {
              //  error_log("测试4下！");
                $sql_check = "UPDATE  `weixin_flag` SET `flag` = '1'  WHERE  `openid` =  '{$from}'";
				$db = mysql::getInstance();
				$data = $db->query($sql_check);
               //mysqli_query($this->dblink,$sql_check);
            }
			//error_log($this->getRequest('msgtype'));
            error_log("测试5下！");
