...
class Goods_mdl extends CI_Model
{
    ...
    public function get_by_id($id, $select = null ,$status = null ,$activity = null)
    {
        ...
        $select .= "p.*, i.width, i.height, i.file,i.is_base, ld.fitst_number, ld.fitst_freight, ld.overflow_number, ld.overflow_freight";
        
        $this->db->select($select);
        $this->db->from('product p');
        $this->db->join('product_image i', 'i.product_id = p.id and i.is_base = 1', 'left');
        $this->db->join('logistics_detail ld', 'ld.logistics_id = p.logistics_id and is_default = 1', 'left');
     ...
     */
    public function get_by_id_forpreview($id)
    {
        if (! $id) {
            return array();
        }
        
        $this->db->select('p.*, i.width, i.height, i.file,i.is_base,ld.fitst_number,ld.fitst_freight,ld.overflow_number,ld.overflow_freight');
        $this->db->from('product p');
        $this->db->join('product_image i', 'i.product_id = p.id and i.is_base = 1', 'left');
        $this->db->join("logistics_detail as ld","ld.logistics_id=p.logistics_id and ld.is_default=1","left");
    ...
    // 获取商品相册
    public function get_gallery($goods_id)
    {
        if (! $goods_id) {
            return array();
        }
        $this->db->where('product_id', $goods_id);
    ...
    public function get_lists_with_condition($limit = 0, $offset = 0, $cate_id, $attrs = array(), $options = array(), $select = "", $wherein = array(), $app_id = 0, $customer_id = 0, $section_id = 0, $corporation_id = 0)
    {
        ...
        if ($select != "") {
            $select = $select . ",i.width, i.height, i.image_name,i.file_ext";
        } else {
    ...
    public function get_shop_pro_lists($limit = 0, $offset = 0, $cate_id,$options = array(),$select = "",$app_id= 0, $shop_id = 0){
        $path = "";
        ...
        if (array_key_exists('order', $options)) {
            switch ($options['order']) {
                case 'sale':
                    ...
                    break;
                case 'new':
                    $this->db->order_by('sp.update_at  DESC');
                    break;
            }
        }
        // 是否深度搜索
        ...
        //已发布
            ...
            $this->db->order_by("sp.sequence DESC");
...
//             }
    ...
    public function get_goods_with_top_category($ids,$perpage=null,$offset=null)
    {
         ...
         * $this->db->where_in('parent_id',$str_ids);
         * $query = $this->db->get();
        ...
        // $this->db->like('c.path', '4324', 'after');
        // $this->db->or_like('c.path', '4415', 'after');
        //echo "test";
        // }
*/
...
//        echo $this->db->last_query().'<br/>';
    ...
    public function get_sub_classification($cate_id){
        ...
        if($row){
            $result = $this->db->select('id')->like('path',$row['path'].',','after')->get('product_cat')->result_array();
            return $result;
        }else{
            return array();
        }
    ...
    function get_productOne($catid,$limit=null,$offset=null,$type=null,$longitude=null,$latitude=null){
        ...
        $this->db->select('a.id,a.cat_id,a.name,a.short_name,a.short_desc,a.vip_price,a.goods_thumb,a.m_price');
...
//         $this->_product_sort($type,$longitude,$latitude);//搜索商品私有方法，根据不同状态排序商品
     ...
     */
    public function get_productFives($limie,$offset){
        $this->db->from("product as a");
        $this->db->where("a.is_on_sale",1);
        $this->db->where("a.is_mc",0);
        $this->db->where("a.is_delete",0);
        $this->db->limit($limie,$offset);
        $this->db->order_by('a.id','desc');
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    /**
     * 搜索商品私有方法，根据不同状态排序商品
     * @param int $type 类型：1,2销量，3,4评论，5,6距离
     * @param string $longitude 经度
     * @param string $latitude 纬度
     */
    private function _product_sort($type,$longitude,$latitude){
        switch ($type){
            case "1"://销量
            case "2":
                $this->db->select('sum(oi.quantity) as sales');
