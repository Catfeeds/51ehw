...
<style type="text/css">
    ...
    .active_nav {/*border-bottom: 2px solid #fed602;padding-bottom: 12px;*/color: #fed602 !important;}
...
<!-- 刷选导航 -->
 ...
 <div class="my-needs-nav" style="padding-bottom: 45px;">
 	<ul>
         <li style="width:30%"><a href="<?php echo site_url("Member/requirement/recommend_demand")?>" style="color:#333333" class="active_nav">精选推荐<span class="icon-fenlei1"></span></a></li>
         <li style="width:25%"><a href="javascript:void(0);">需求总价</a></li>
	     <li style="width:25%"><a href="javascript:void(0);">发布时间</a></li> 
...
dropload = $('#main').dropload({
	...
	loadDownFn : function(me){
		 ...
		 $.post("<?php echo site_url("Member/requirement/ajaxload_requirement");?>",{page:page,keyword:keyword,cate_id:cate_id,orderBy:type},function(data){
			 if(data.demandlist.length>0){
				 ...
				 // for(var i=0;i<data.demandlist.length;i++){
					...
					for(var i=0;i<data.demandlist.length;i++){
						...
						// console.log(data.demandlist[i]['create_by']);
						...
						result += '<li>';
							result += '<a href="'+url+'">';
								// result += '<div><a ><img src="'+'<?php //echo base_url(),'uploads/B/';?>'+data.demandlist[i]['img_path']+'"></a></div>';
						
								result += '<div class="my_needs_goods_img" style="width: 25%; height: 89px;"><img src="'+img+'" onerror="'+errorimg+'" alt=""></div>';
							
								result += '<div class="my_needs_text">';
									result += '<span class="my-needs-li-title">'+data.demandlist[i]['title']+'</span>';
									result += '<span class="my-needs-li-money">总价：'+data.demandlist[i]['total_price']+'券</span>';
								result += '</div>';
							result += '</div>';
