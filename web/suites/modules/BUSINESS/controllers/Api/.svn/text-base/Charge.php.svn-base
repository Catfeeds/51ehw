...
class Charge extends Api_Controller {
    ...
    public function purchase_M()
    {
        ...
        $status =  json_decode($this->curl_post_result($url,$data),true);
...
//             $return['responseMessage'] = array('messageType'=>'error','errorType'=>'9','errorMessage'=>'订单已支付');
    ...
    public function charge_pay(){
        ...
        if($is_order){
            ...
            //需要支付的手续费
            $pay_commission = empty($is_order['commission']) ? 0.00 :  $credit['cash'] >= $is_order['commission'] ? 0.00 :  $is_order['commission'] - $credit['cash'] ;
            
            if($available_amount >= $is_order['total_price']){
                $amount = $pay_commission;
            }else{
                $amount =round($is_order['total_price'] - $available_amount,2) + $pay_commission;
            }
            
            if(empty($paypassword)){
                if($amount != $is_order['commission']+$is_order['total_price']){//当充值的金额不等于订单跟手续费的总和 则需要验证密码
                    if($pay_account['pay_passwd'] != md5($paypassword)){
                        $return['responseMessage'] = array('messageType'=>'error','errorType'=>'4','errorMessage'=>'支付密码错误');
                        print_r(json_encode($return));
                        exit;
