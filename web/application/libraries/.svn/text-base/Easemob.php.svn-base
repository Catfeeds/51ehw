...
//------------------------------------------------------用户体系	
	...
	*/
	...
	function mkdirs($dir, $mode = 0777)
	 {
	     $path = FCPATH.UPLOAD_PATH.'uploads/'.$dir;
	     if ( !file_exists( $path ) )
	     {
	...
	function writeCursor($filename,$content){
		//判断文件夹是否存在，不存在的话创建
		if(!file_exists("chat/txtfile")){
			$this->mkdirs("chat/txtfile");
		}
		$myfile=@fopen("chat/txtfile/".$filename,"w+") or die("Unable to open file!");
	...
	*/
	...
	/*
		下载文件或图片
	*/
	function downloadFile($uuid,$shareSecret,$download_url){
		...
		//生成文件夹
		$this->mkdirs($download_url);
		$path = FCPATH.UPLOAD_PATH.'uploads/'.$download_url;
	...
	*/
	function downloadThumbnail($uuid,$shareSecret){
		...
		if(!file_exists("chat/down")){
			//mkdir("../image/down");
			$this->mkdirs("chat/down/");
		}
		
		$file = @fopen("chat/down/".$filename,"w+");//打开文件准备写入
		@fwrite($file,$result);//写入
		fclose($file);//关闭
		return $filename;
	}
