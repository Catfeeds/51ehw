        ...
        if ($tribe_id == '0') {
            switch ($types) {
                case "1":
                    ...
                    where a.product_name like '%".$this->db->escape_like_str($keyword)."%' 
                ...
                case "3":
                    ...
                    and type = 1 
                    where product_name like '%".$this->db->escape_like_str($keyword)."%'
                    order by price ASC
                    limit $offest,$limit");
                    return $query->result_array();
                    break;
            ...
            switch ($types) {
                case "1":
                    ...
                    left join  9thleaf_easy_order as b on a.id  = b.product_id and b.status in ('4','5')
                    left join 9thleaf_easy_product_img as c on a.id = c.product_id and c.type = 1 
                    where a.tribe_id = $tribe_id and a.product_name like '%".$this->db->escape_like_str($keyword)."%'
                    group by a.id  order by volume asc limit $offest,$limit");
                    return $query->result_array();
                    break;
                case "2":
                ...
                case "3":
                    ...
                    left join 9thleaf_easy_product_img as b on a.id = b.product_id
                    and type = 1
                    where tribe_id = $tribe_id and product_name like '%".$this->db->escape_like_str($keyword)."%'
                    order by price ASC
                    limit $offest,$limit");
                    return $query->result_array();
                    break; 
...
//         $this->db->where('easy_product_img.type','0');
