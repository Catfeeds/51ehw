        ...
        <div class="clearfix">
            ...
            <div class="product">
                ...
                <lable class="item" id="special_price_item" style="display:none;">
                    ...
                    <span style="font-size: 15px; font-weight: bold; color: red;" id="special_price"> <?php echo number_format($details['special_price'],2); ?> M券 </span>
                </lable>
                <lable class="item" id="product_price_item">
                    易货价<em style="opacity: 0;">测</em>
                    <span class="red" id="product_price" style="color: #333333; font-size: 15px; font-weight: bold;"> <?php echo $details['vip_price'] == 0 ? "0":number_format($details['vip_price'], 2);?> M券</span>
                </lable>
               ...
               <span class="add-del">
                ...
                <lable class="item">小计<em style="opacity: 0;">测试</em>
                    <span class="red" id="total_price" style="color: #333333; font-size: 15px; font-weight: bold;"> <?php echo $details['vip_price'] == 0 ? "0":number_format($details['vip_price'], 2);?> M券</span>
                </lable>
                <p class="caution_tips" id="item-error" style="display: none;"></p>
                <!-- 运费 -->
                <lable class="item">运费<em style="opacity: 0;">测试</em><span class="red" id="freight" style="color: #333333;font-size: 15px;margin-left: 3px;"><?php echo $details['is_freight'] ? $details['default_freight'].' M券' : '免运费'?></span></lable>
...
function account(obj){
    ...
    $("#total_price").text(formatCurrency((special_price.replace(/,/g,"") * x).toFixed(2)) +" M券");
    ...
    $("#total_price").text(formatCurrency(sku_special_offer *$(obj).val()) + " M券");
    <?php }else{?>
    $("#total_price").text(formatCurrency((curr_price.replace(/,/g,"") * x).toFixed(2)) +" M券");
    <?php };?>
}
//商品数量操作结束
...
{
	window.location.href="<#name#>name=<?php echo $details['name'].'&desc=我在51乐赚发现了一款不错的商品，赶快来看看吧!&img='.IMAGE_URL.$details['goods_img'];?>";
}

//判断是否互助店商品
function is_shop_pro(){
	 window.location.href="<#communal#>id=<?php echo $communal;?>";
}
//分享互助店商品
function share(){
...
if(date ('Y-m-d H:i:s')>=$special_price_start_at && date ('Y-m-d H:i:s')<=$special_price_end_at){  $special_price_item = true;//判断是否特价?>
    ...
    $('#total_price').html((special_price*$('#item_num').val()).toFixed(2)+" M券");
<?php
}
...
function setInfo()
{
    ...
    for(var i=0;i<skuprice.length;i++)
    {
        ...
        {
            $("#product_price").html((formatCurrency(skuprice[i][2]))+" M券");
            ...
            $("#special_price_item").show();
            $("#product_price_item").css("text-decoration","line-through");
            $('#total_price').html((formatCurrency(skuprice[i][4]*$('#item_num').val())) +" M券");
            $('#special_price').html(formatCurrency(skuprice[i][4]) +" M券");
            <?php else:?>
            $('#total_price').html((formatCurrency(skuprice[i][2]*$('#item_num').val())) +" M券")
            <?php endif;?>
            break;
        }
    }
    ...
    if (parseInt(x) > parseInt(max)) {
        ...
        $("#total_price").text(formatCurrency((special_price.replace(/,/g,"") * x).toFixed(2)) +" M券");
        <?php }elseif(!empty($skuinfo['skuinfo']) && !empty($skuinfo['skulist']) && !empty($skuinfo['skuitem']) && $details['special_price_start_at'] <= date("Y-m-d H:i:s") && $details['special_price_end_at'] >= date("Y-m-d H:i:s")){?>
        $("#total_price").text(formatCurrency(sku_special_offer * x) + " M券");
        <?php }else{?>
        $("#total_price").text(formatCurrency((curr_price.replace(/,/g,"") * x).toFixed(2)) +" M券");
        <?php };?>
    }
...
function add_to_cart(pid,pname,imgurl,cor_id,cor_name)
{
    ...
    window.location.href="<#add_to_cart#>id=" + pid + "&qty=" + qty+"&price="+special_price+"&pname="+pname+"&imgurl="+imgurl+"&&val_id="+sku_id+"&cor_id="+cor_id+"&cor_name="+cor_name+"&attr_sku="+attr_sku+"&freight="+freight;
    <?php }else{?>//传正常商品价格
    window.location.href="<#add_to_cart#>id=" + pid + "&qty=" + qty+"&price="+curr_price+"&pname="+pname+"&imgurl="+imgurl+"&&val_id="+sku_id+"&cor_id="+cor_id+"&cor_name="+cor_name+"&attr_sku="+attr_sku+"&freight="+freight;
	<?php }?>
}
...
function buy(pid,pname,imgurl,cor_id,cor_name)
{
	...
	if(sku_id){
		window.location.href="<#buy#>id=" + pid + "&qty=" + qty+"&pname="+pname+"&imgurl="+imgurl+"&price="+sku_special_offer+"&val_id="+sku_id+"&cor_id="+cor_id+"&cor_name="+cor_name+"&attr_sku="+attr_sku+"&freight="+freight;
	}else{
		window.location.href="<#buy#>id=" + pid + "&qty=" + qty+"&pname="+pname+"&imgurl="+imgurl+"&price="+special_price+"&val_id="+sku_id+"&cor_id="+cor_id+"&cor_name="+cor_name+"&attr_sku="+attr_sku+"&freight="+freight;
	}
    <?php }else{?>//传正常商品价格
    window.location.href="<#buy#>id=" + pid + "&qty=" + qty+"&pname="+pname+"&imgurl="+imgurl+"&price="+curr_price+"&val_id="+sku_id+"&cor_id="+cor_id+"&cor_name="+cor_name+"&attr_sku="+attr_sku+"&freight="+freight;
    <?php }?>
}
...
function count_freight(num){
    ...
    if(num > default_item ){
        ...
        if(isInteger(freight) ){ 
            $('#freight').text(freight+'.00 M券');
        }else{ 
           $('#freight').text(freight+' M券');
        }
        return freight;
    }else{ 
        $('#freight').text(default_freight+' M券');
	 ...
	 wx.onMenuShareAppMessage({
	  ...
	  // 2.1 监听“分享给朋友”，按钮点击、自定义分享内容及分享结果接口
	    ...
	    wx.onMenuShareAppMessage({
	      ...
	      link: '<?php echo site_url('shop/skipping')."?id=".$communal."&time".$time."&type=".$type."&parent=".$parent."&mark=".base64_decode(3)?>',
...
// 	    alert('已注册获取“发送给朋友”状态事件');
	  ...
	  // 2.2 监听“分享到朋友圈”按钮点击、自定义分享内容及分享结果接口
	    ...
	    wx.onMenuShareTimeline({
	   	  title: '<?php echo $details['name'];?>',
		  link: '<?php echo site_url('shop/skipping')."?id=".$communal."&time".$time."&type=".$type."&parent=".$parent."&mark=".base64_encode(3);?>',
		  imgUrl: '<?php echo IMAGE_URL.$details['goods_thumb'];?>',
	      trigger: function (res) {
	        // 不要尝试在trigger中使用ajax异步请求修改本次分享的内容，因为客户端分享操作是一个同步操作，这时候使用ajax的回包会还没有返回
// 	        alert('用户点击分享到朋友圈');
