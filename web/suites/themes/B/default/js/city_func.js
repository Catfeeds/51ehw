...
function residencySelect(){
	...
	var dragHtml ='<div id="residencyAlpha">';		//居住地
		...
		dragHtml+='</div>';
	$('#drag_h').html('<span onclick="boxAlpha()">关闭</span>');
...
var jobArea = {
				...
				for (var i in arr){
					...
					for (var j in arr[i][1] ){
						...
						if(k==0){
							...
							if(area=='其它'){
								 output+='<li class="jobArea'+id+'" onclick="jobArea.Chk(\''+id+'\')">'+ja[id]+'</li>';
							}else{ 
								output+='<li onclick="jobArea.SubLayer(\''+id+'\')">'+ja[id]+'</li>';
							}
						}				
					}
					output+='</dd></dl>';
	...
	confirm : function(){
		...
		areaStr=areaStr.substring(1)?areaStr.substring(1):'未添加地区'; 
...
/*if(typeof $("#jobAreaID").val()=='string'&&""!=$("#jobAreaID").val()){
		var sp=$("#jobAreaID").val().split(",");
		$.each(sp,function(index,item){
			jobArea_Arr[index]=item;
		})
	}*/
