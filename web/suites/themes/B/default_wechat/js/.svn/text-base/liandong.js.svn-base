...
//ajax获取地址
$.post(base_url+"/region_change/second_level",function(data){
	var cityData = "[";
	for(var i=0;i<data.length;i++){
		cityData += '{"id":"'+data[i]['region_id']+'","name":"'+data[i]['region_name']+'","shortName":"'+data[i]['region_name']+'","parentId":"'+data[i]['parent_id']+'","level":"'+data[i]['region_type']+'"},';
	}
	cityData = (cityData.substring(cityData.length-1)==',')?cityData.substring(0,cityData.length-1):cityData;
	cityData += "]";
...
$(function(){
    ...
    var $mask=$('<div class="mPicker-mask hide_yin"></div>');

    var $mPicker=$('<div class="mPicker hide_yin"></div>');
    ...
    MPicker.prototype={
        ...
        showPicker:function(){
            ...
            self.container.blur();
            self.mpicker.removeClass('hide_yin');
            self.mask.removeClass('hide_yin');
        ...
        confirm:function(){
            ...
            self.hide_yinPicker(self.options.confirm);

        },
        cancel:function(){
            var self=this;
            self.hide_yinPicker(self.options.cancel);
        },
         /**
         *  事件
         *  取消，确定，点击遮罩，列表滑动事件
...
function mPickercancel(){}//自定义取消方法，可以覆盖重写
function mPickerconfirm(){};//自定义确定方法，可以覆盖重写