...
class alipay
{
    ...
    function respond()
    {
		...
		$payment = $CI->payment_mdl->get_payment($_GET['code']);
        $order_sn = trim($_GET['subject']);
        
		$CI->load->model('order_mdl');
        /* 检查支付的金额是否相符 */
        if (!$CI->order_mdl->check_money($order_sn, $_GET['total_fee']))
        {		
            return false;
        }
        ...
        {
            /* 改变订单状态 */
            $CI->order_mdl->order_paid($order_sn);
