...
//         $_data['member_name'] = $this->input->get_post("member_name");
        ...
        //通过手机验证成员信息
        $staff_info =  $this->tribe_mdl->verify_tribe_user($tribe_id, $_data['mobile']);
        ...
        if(!$staff_info){
            ...
            $aff =  $this->tribe_mdl->update_member($update,$tribe_id);
            
            //同步部落身份信息
            $staff_idenity =  $this->tribe_mdl->load_staff_idenity($_data['mobile']);
            if($staff_idenity){
            ...
            $this->Tribe_staff_album_mdl->synchro_Update($user_id,$tribe_id);
            
            $num = 3;//提示改成3次
            $this->tribe_mdl->update_tips($num,$_data['mobile']);
        }
       ...
       if($row){
           ...
           if($staff_info){
               ...
               if($staff_info['status'] != 2 ){
                   ...
                   $this->load->model('tribe_mdl');
                   $affs = $this->tribe_mdl->update_tribe_staff( $update_data );
               }
               if($type && $type = "json"){
                   echo json_encode($return);exit;
               }else{
...
//            $update_data['status'] = 2;
           ...
           if($update_aff){
               ...
               if(!$staff_info){
                   ...
                   //通过手机验证成员信息
                   $entering_staff_info =  $this->tribe_mdl->verify_tribe_user($package_info['tribe_id'],$BindingMobile);
                   if(!$entering_staff_info){
                       ...
                       $update['add'] = true;
                       $update['id'] = $entering_staff_info['id'];
                       $update['customer_id'] = $customer_id;
                       $aff =  $this->tribe_mdl->update_member($update,$package_info['tribe_id']);
                       
                       //同步部落身份信息
                       $staff_idenity =  $this->tribe_mdl->load_staff_idenity($BindingMobile);
                       if($staff_idenity){
                           foreach ($staff_idenity as $key =>$val){
                               $this->tribe_mdl->del_staff_idenity($val['id']);
                               unset( $staff_idenity[$key]['id']);
                       ...
                       ///同步部落预录入的相册个人形象
                       $this->load->model('Tribe_staff_album_mdl');
                       $this->Tribe_staff_album_mdl->synchro_Update($customer_id,$package_info['tribe_id']);
                       $num = 3;//提示改成3次
                       $this->tribe_mdl->update_tips($num,$BindingMobile);
                   }
                   if($aff){
                       $is_ok =true;
                   }
