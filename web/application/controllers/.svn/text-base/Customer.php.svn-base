...
class customer extends Front_Controller
{
    ...
    function save()
    {
        ...
        if (TRUE == $this->validation->run()) {
            ...
            if (stristr($_SERVER['HTTP_USER_AGENT'], "Android") || stristr($_SERVER['HTTP_USER_AGENT'], "iPhone") || stristr($_SERVER['HTTP_USER_AGENT'], "wp")) {
                $this->customer_mdl->registry_by = "H5";
            }
            ...
            if (isset($step) && $step == 1) {
                ...
                $this->session->set_userdata('mobile_verfity');
                if ($this->is_mobile > 0) {
                    redirect($refUrl);
                } else {
            ...
            if (isset($inviteid)) {
                ...
                redirect('customer/registration', $data);
            }
        }
    }
    ...
    public function userquan()
    {
        ...
        $this->load->view('head', $data);
        $this->load->view('_header', $data);
        $this->load->view('customer/userquan', $data);
        $this->load->view('_footer', $data);
        $this->load->view('foot', $data);
    }
    ...
    public function fortune()
    {
        ...
        $data['customer'] = $this->pay_account->load($account_id);
        if ($data['customer']) {
            if (! ($data['customer']['credit_start_time'] <= $time && $data['customer']['credit_end_time'] >= $time)) {
    ...
    {
        $this->session->set_userdata('forget');
        ...
        if ($cus != null) {
            // 判断条件是否满足
            if ($name != null && $password != null && $ConfirmPassword != null && $ConfirmPassword == $password) {
                
                $this->load->model('customer_mdl');
                $this->customer_mdl->password = md5($password);
                $res = $this->customer_mdl->forget_password($cus['name']);
                $this->session->set_userdata('password_mobile_verfity');
