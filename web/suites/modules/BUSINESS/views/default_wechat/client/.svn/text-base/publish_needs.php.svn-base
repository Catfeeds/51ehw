...
#city{ width:70%;padding: 5px 0;border:1px solid #fff; background:none; outline:none;margin-top: -4px;float: right;text-align: right;font-size: 13px; }
#PoPy{ width:100% !important; left:0px !important;}
.webuploader-pick { background: #62BB10;width: 140px;} 
.webuploader-container { position: static; }
...
.needs_fenlei {float: right;color: #666666;font-size: 13px;}
 ...
 <div class="offer_details">
	...
	<div class="publish-needs-main">
		<ul class="offer-details-main-ul02 border-none">
		    ...
		    <li><span>需求标题：</span><input type="text" value="" id="title" class="publish-needs-input-title" placeholder="请输入标题内容"></li>
		   ...
		   <!--   <li><span>交货期(天)：<span style="opacity: 0;">测</span></span><input type="text"  value="" class="publish-needs-input"></li>-->
		     ...
		     <li><span>报价截止时间：</span>
		                <span class="income_total_time">
							...
							<input onclick="WdatePicker()" id="effectdate" name="start" value="" placeholder="请选择时间" readonly="" class="publish-needs-time">
		     ...
		     <li><span>收货日期：
		                    ...
		                    <span class="icon-right fn-right" style="padding-top: 2px;color: #666;"></span>		
							<input onclick="WdatePicker()" id="receiptdate" name="end" value="" placeholder="请选择时间" readonly="" class="publish-needs-time">
						</span>
			</li>
		     <li class="needs-offer-qita"><span>报价须含 (可不选) :</span>
				<a href="javascript:changeval(1);"><i id="freights" class="icon-round icon-roundcheck"></i>运费</a>
				<a href="javascript:changeval(2);"><i id="taxs" class="icon-round icon-roundcheck"></i>税</a>
				<input type="hidden" value="0" id="freight"><input type="hidden" value="0" id="tax">
		    </li>
		    <li><span>收货地址:</span>
		              <span class="icon-right fn-right" style="padding-top: 2px;color: #666;"></span>
		              <input type="button" id="city" value="请选择地区">
		               <input type="button" id="province_id" value="" hidden>
		                <input type="button" id="city_id" value="" hidden>
		                 <input type="button" id="district_id" value="" hidden>
		              		
		    </li>
		    <li><input type="text" value="" id="shippingaddress" class="publish-needs-address" placeholder="详细地址" style="width: 100%;"></li>
		    ...
		    <li><span>产品图片:<span style="opacity: 0;">测试</span></span>
		         <span>
                   ...
                   <input type="button" onclick="file.click();"   class="publish-needs-button02" style="width:65%;" value="选择上传图片">
...
function sub(){
	...
	var r= /^[+-]?[1-9]?[0-9]*\.[0-9]*$/;
	if(r.test(p_count)){
		if(p_count.toString().split(".")[1].length >2){
			 $(".black_feds").text("数量只允许保存到小数点后两位！").show();
				setTimeout("prompt();", 2000);
		     	return;
			}
		}
	...
	if(r.test(m_price)){
    	if(m_price.toString().split(".")[1].length >2){
    		 $(".black_feds").text("单价只允许保存到小数点后两位！").show();
    			setTimeout("prompt();", 2000);
    	     	return;
    		}
	}
    ...
    (
        {
            ...
            success: function (data, status)  //服务器成功响应处理函数
            {
                ...
                $.ajax({
                	url:'<?php echo site_url("member/requirement/ajax_addrequirement") ?>',
        		    dataType:'json',
        		    type:'post',
        		    data:{ title:title,cateid:cateid,
            		    p_count:p_count,m_price:m_price,unit:unit,receiptdate:receiptdate,effectdate:effectdate,freight:freight,
            		    tax:tax,province_id:province_id,city_id:city_id,district_id:district_id,shippingaddress:shippingaddress,
        		    	img_path:img_path      
        		    },
...
function preImg(sourceId, targetId) {  
    ...
    if(size > 1024*1024*2){
    	$(".black_feds").text("图片大小不能超过2M！").show();
		setTimeout("prompt();", 2000);
		return;
        }
