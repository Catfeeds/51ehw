...
class Notify_url extends Front_Controller {
    ...
    public function charge_pay($chargeID = null){
        ...
        if(!$chargeID){
            ...
            $charge_id = $this->session->userdata ( 'Acppay_chargeNo' );
            if(!$charge_id){
                redirect("Member/info");exit;
            }
            $this->session->unset_userdata('Acppay_chargeNo');//清除
            $charge = $this->charge->load ( $charge_id );
             
            //更改成银联支付
            $this->charge->update_payment( $charge_id, 3 );
            //把时间稍微转换下如20170720110712
            $pay_date = str_replace(array("-",":"," "),"",$charge['pay_date']);
            //平台以元味基础单位，因银联以分为基础单位，也要转换下
            $amount = $charge['amount']*100;
            $this->FrontConsume($charge['chargeno'],$pay_date,$amount);
        }else{
            ...
            $this->FrontConsume('POR'.$charge_info['chargeno'],$pay_date,$amount);
        }
                    ...
                    if($data['status'] == 0 ){
                        ...
                        if($amount == $data['txnAmt']){
                            if ($pay_type == "ODR") { //拼团订单的
                                $this->after_groupbuy($orderId,$trade_no);
                            }else if ($pay_type == "CHR"){ //现金充值 
                                $is_ok = $this->after_pay($orderId,$trade_no);
    				        }else if($pay_type =='COR'){ //微信二维码方式的充值
                                $is_ok = $this->after_pay($orderId,$trade_no);
                            }else if($pay_type == 'POR'){ //普通订单的 
    				            $is_ok = $this->after_order($orderId,$trade_no);
    				        }else{
    				            $is_ok = $this->after_pay($orderId,$trade_no);
    				        }
                        }else{
    ...
    private function after_pay($out_trade_no,$trade_no){
        ...
        if($charge && $trade_no ){
            ...
            // 修改订单状态为已支付
            $charge_row = $this->charge_mdl->chargeno_update_pay ( $out_trade_no,'银联订单号:'.$trade_no );
    ...
    //拼团支付返回逻辑
    private function after_groupbuy($out_trade_no, $trade_no){
