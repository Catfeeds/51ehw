        ...
        <div class="clearfix">
            ...
            <div class="product">
                ...
                <?php if($is_special){?>
                    ...
                    <span style="font-size: 15px; font-weight: bold; color: red;" id="special_price"> 
                    <?php echo number_format($details['special_price'],2); ?> 货豆 </span>
               	 	</lable>
                <?php }else if($tribeVIP){?>
                     <lable class="item" style="margin-top: 5px;">部落价&nbsp;&nbsp;
                        <span class="red" style="font-size: 15px;font-weight: bold; color: red;" id="tribe_price">
                        <?php  echo $curr_price = number_format($details['tribe_price'], 2);?> 货豆
                    	</span>
                     </lable>
                <?php }
                    if($details['cat_id'] != 104063){?>
                     <lable class="item" id="product_price_item">易货价<em style="opacity: 0;">测</em>
                    <span class="red" id="product_price" style="color: #333333; font-size: 15px; font-weight: bold;"> 
                    <?php if($is_special || $tribeVIP){   ?>
                        <del><?php echo $details['vip_price'] == 0 ? "0":number_format($details['vip_price'], 2);?> 货豆</del>
                   <?php }else{ ?>
                       <?php echo $details['vip_price'] == 0 ? "0":number_format($details['vip_price'], 2);?> 货豆
                    <?php }?>
                    </span>
                    </lable>
                <?php  }?>
                ...
                <?php };?>  
                 <!-- <input type="hidden" value="<?php echo $curr_price;?>" id="curr_price">商品单价 -->
                <div class="line"></div>
                <?php if($details['cat_id'] != 104063){?>
               ...
               <span class="add-del">
                ...
                <!-- 小计 -->
                <lable class="item">小计<em style="opacity: 0;">测试</em>
                    <span class="red"  style="color: #333333;font-size: 15px;font-weight: bold;padding-left:8px;" id="total_price">
                ...
                <!-- 运费 -->
                <?php if($details['cat_id'] != 104063){?>
                <lable class="item">运费<em style="opacity: 0;">测试</em><span class="red" id="freight" style="color: #333333;font-size: 15px;margin-left: 3px;"><?php echo $details['is_freight'] ? $details['default_freight'].' 货豆' : '免运费'?></span></lable>
				<?php }?>

                  <!-- 店铺名 -->
                <div class="shop-name" style="border-bottom: none;">
...
function account(obj){
    var x = jQuery(obj).val();
    <?php if($is_special && empty($skuinfo['skulist']) && empty($skuinfo['skuitem'])){?>//判断是否特价商品
    $("#total_price").text(formatCurrency((special_price.replace(/,/g,"") * x).toFixed(2)) +" 货豆");
    <?php }elseif(!empty($skuinfo['skuinfo']) && !empty($skuinfo['skulist']) && !empty($skuinfo['skuitem']) && $is_special){?>
...
var tribe_price = 0;//部落价
<?php if($is_special){//特价 ?>
	var curr_price = '<?php echo $details['special_price'] == 0 ? "0":$details['special_price'];?>';
<?php }else if($tribeVIP){//部落价 ?>
	var curr_price = '<?php echo $details['tribe_price'] == 0 ? "0":$details['tribe_price'];?>';
<?php }else{ //易货价 ?>
...
?>
skuprice[<?php echo $key;?>] = ['<?php echo $list["sku_key"]?>',<?php echo $list["store"];?>,<?php echo $list["m_price"];?>,<?php echo $list["val_id"];?>,<?php echo $list["special_offer"];?>,<?php echo $list["tribe_price"];?>];
<?php
endforeach;
...
function setInfo()
{
        ...
        if(skuprice[i][0]+"_" == item)
        {
         ...
         <?php if($is_special){?>
            $("#special_price").html((formatCurrency(skuprice[i][4]))+" 货豆");
         	$("#product_price").html("<del>"+(formatCurrency(skuprice[i][2]))+" 货豆</del>");
         	$("#total_price").html((formatCurrency(skuprice[i][4]))+" 货豆");
         	curr_price = formatCurrency(skuprice[i][4]);
         <?php }else if($tribeVIP){?>
         	$("#product_price").html("<del>"+(formatCurrency(skuprice[i][2]))+" 货豆</del>");
         	$("#total_price").html((formatCurrency(skuprice[i][5]))+" 货豆");
        	$("#tribe_price").html((formatCurrency(skuprice[i][5]))+" 货豆");
         	curr_price = formatCurrency(skuprice[i][5]);
         <?php }else{ ?>
    ...
    if (parseInt(x) > parseInt(max)) {
        ...
        <?php if($is_special && empty($skuinfo['skuinfo']) && empty($skuinfo['skulist']) && empty($skuinfo['skuitem'])){?>//判断是否特价商品
        $("#total_price").text(formatCurrency((special_price.replace(/,/g,"") * max).toFixed(2)) +" 货豆");
        <?php }elseif(!empty($skuinfo['skuinfo']) && !empty($skuinfo['skulist']) && !empty($skuinfo['skuitem']) && $is_special){?>
        $("#total_price").text(formatCurrency(sku_special_offer * max) + " 货豆");
        <?php }else{?>
        $("#total_price").text(formatCurrency((curr_price.replace(/,/g,"") * max).toFixed(2)) +" 货豆");
        <?php };?>
    }else{
...
function add_to_cart(pid,pname,imgurl,cor_id,cor_name)
{
    ...
    <?php if($is_special){?>//特价商品期间传商品特价价格
    //当价格有逗号时，则去去除逗号
    special_price = String(special_price).replace(",","");
    window.location.href="<#add_to_cart#>id=" + pid + "&qty=" + qty+"&price="+special_price+"&pname="+pname+"&imgurl="+imgurl+"&&val_id="+sku_id+"&cor_id="+cor_id+"&cor_name="+cor_name+"&attr_sku="+attr_sku+"&freight="+freight+"&commission_rate="+<?php echo $rebate?>;
    <?php }else{
        if($tribeVIP){ ?>//传部落商品价格
       // curr_price = '<?php  //echo number_format($details['tribe_price'], 2)?>';
        curr_price = String(curr_price).replace(",","");
        window.location.href="<#add_to_cart#>id=" + pid + "&qty=" + qty+"&price="+curr_price+"&pname="+pname+"&imgurl="+imgurl+"&&val_id="+sku_id+"&cor_id="+cor_id+"&cor_name="+cor_name+"&attr_sku="+attr_sku+"&freight="+freight+"&commission_rate="+<?php echo $rebate?>;
    <?php  }else{?>//传正常商品价格
    //当价格有逗号时，则去去除逗号
    curr_price = String(curr_price).replace(",","");
	window.location.href="<#add_to_cart#>id=" + pid + "&qty=" + qty+"&price="+curr_price+"&pname="+pname+"&imgurl="+imgurl+"&&val_id="+sku_id+"&cor_id="+cor_id+"&cor_name="+cor_name+"&attr_sku="+attr_sku+"&freight="+freight+"&commission_rate="+<?php echo $rebate?>;
...
function buy(pid,pname,imgurl,cor_id,cor_name)
{
    ...
    if(sku_id){
        //当价格有逗号时，则去去除逗号
    	sku_special_offer = String(sku_special_offer).replace(",","");
		window.location.href="<#buy#>id=" + pid + "&qty=" + qty+"&pname="+pname+"&imgurl="+imgurl+"&price="+sku_special_offer+"&val_id="+sku_id+"&cor_id="+cor_id+"&cor_name="+cor_name+"&attr_sku="+attr_sku+"&freight="+freight+"&commission_rate="+<?php echo $rebate?>;
	}else{
		 //当价格有逗号时，则去去除逗号
		special_price = String(special_price).replace(",","");
		window.location.href="<#buy#>id=" + pid + "&qty=" + qty+"&pname="+pname+"&imgurl="+imgurl+"&price="+special_price+"&val_id="+sku_id+"&cor_id="+cor_id+"&cor_name="+cor_name+"&attr_sku="+attr_sku+"&freight="+freight+"&commission_rate="+<?php echo $rebate?>;
	}
    <?php }else{
    if($tribeVIP){ ?>//传部落商品价格
    //curr_price = '<?php // echo number_format($details['tribe_price'], 2)?>';
    curr_price = String(curr_price).replace(",","");
    window.location.href="<#buy#>id=" + pid + "&qty=" + qty+"&pname="+pname+"&imgurl="+imgurl+"&price="+curr_price+"&val_id="+sku_id+"&cor_id="+cor_id+"&cor_name="+cor_name+"&attr_sku="+attr_sku+"&freight="+freight+"&commission_rate="+<?php echo $rebate?>;
    <?php  }else{?>//传正常商品价格
    //当价格有逗号时，则去去除逗号
    curr_price = String(curr_price).replace(",","");
    window.location.href="<#buy#>id=" + pid + "&qty=" + qty+"&pname="+pname+"&imgurl="+imgurl+"&price="+curr_price+"&val_id="+sku_id+"&cor_id="+cor_id+"&cor_name="+cor_name+"&attr_sku="+attr_sku+"&freight="+freight+"&commission_rate="+<?php echo $rebate?>;
