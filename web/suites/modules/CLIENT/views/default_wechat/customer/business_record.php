  ...
  <div class="container" style="background-color:#e8e8e8">
     <div class="business_record_nav">
          <ul>
              ...
              <li class="business_record_nav03">授信额度</li><span>｜</span>
              <li class="business_record_nav04">储值卡</li>
          </ul>
     </div>
     ...
     <div class="business_record_list">
          ...
          <!-- 授信结束 -->
           ...
           <!-- 卡开始 -->
          <?php if( count($card_log) > 0 ):?>
          <ul class="business_record_list_04">
              <?php foreach ($card_log as $v):?>
                  <li>
                    <p><span><?php echo $v['remarks']?></span><span class="business_record_list_num1"><?php echo $v['type'] == 1 ? '+'.$v['card_amount'] : '-'.$v['card_amount']?></span></p>
                    <p><span><?php echo $v['created_at']?></span></p>
                  </li>
              <?php endforeach;?>
          </ul>
  ...
  <input type="hidden" value="2" id="limit_4">
...
//                           	  html += '<p>收款方：<span>'+data.log[i]['corporation_name']+'</span></p>'
                	  ...
                	  for(var i=0;i< data.log.length;i++){
                    	   var form_or_to = data.log[i]['type'] == 1 ? '+' : '-';
                          html += '<li><p><span>'+data.log[i]['remarks']+'</span><span class="business_record_list_num1">'+form_or_to+data.log[i]['card_amount']+'</span></p><p><span>'+data.log[i]['card_amount']+'</span></p> </li>';
                     } 
                  }
  ...
  $(function(){
    ...
    $(".business_record_nav04").on("click",function(){
      $(".business_record_list_01").hide()
      $(".business_record_list_02").hide();
      $(".business_record_list_03").hide();
      $(".business_record_list_04").show();
      $('#status').val(4);
      $('#panduan').val(1);
    })
