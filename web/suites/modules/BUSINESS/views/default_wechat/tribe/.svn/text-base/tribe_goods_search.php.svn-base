...
dropload = $('#sort').dropload({
	...
	loadDownFn : function(me){
	    // 加载数据
        var result = "";
        $.post("<?php echo site_url("tribe/ajax_search_goods");?>",{keyword:keyword,type:types,page:page,navigate:navigate,tribe_id:tribe_id},function(data){
            ...
            if(data["productList"].length>0){
                ...
                for(var i=0;i<data["productList"].length;i++){
                	...
                	result += '<a href="<?php echo site_url('tribe/good_detail');?>/'+data["productList"][i]['id']+'"><li class=" clearfix" style="">';
