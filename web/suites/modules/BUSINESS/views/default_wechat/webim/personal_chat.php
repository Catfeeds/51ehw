...
<div class="personal_chat">
	...
	<div class="personal_chat_head">
		...
		<a href="<?php echo site_url("Tribe_social/Customer_Info/{$to_customer['id']}?tribe_id={$tribe_id}&ts_id={$to_customer['ts_id']}");?>" class="icon-icon_my_on"></a>
    	    ...
    	    if($Msg_list){
    	       ...
    	       foreach ($Msg_list as $key =>$val){
    	           ...
    	           if($val['from_customer_id'] == $from_customer['id']){
    	              ...
    	              $matter_str .= '<li>';
    	              $matter_str .= '<div class="personal_chat_time" '.$show.' ><span>'.substr($val['create_at'],0,16).'</span></div>';
    	              $matter_str .= '<div class="personal_chat_img"><img src="'.$to_customer['logo'].'"  onerror="'.$error_defult_img.'" alt=""></div>	';
    	               ...
    	               if(data["list"][i]['from_customer_id'] == '<?php echo $from_customer['id'] ?>'){
    	            	   ...
    	            	   result_str += '<div class="personal_chat_content fn-right">';
        	            	  ...
        	            	  result_str += '  </li>';
        	            	   ...
        	            	   result_str += '<li >';
        	            	   result_str += '<div class="personal_chat_time" '+show+'><span>'+data["list"][i]['create_at'].substr(0,16)+'</span></div>';
        	            	   result_str += '<div class="personal_chat_img"><img src="<?php echo $to_customer['logo'];?>"  onerror="'+error_defult_img+'" alt=""></div>	';
...
function sendText(parms,send){
	...
	if(!parms){
	 ...
	 $.ajax({ 
		    ...
		    type:'post',
		    dataType:'json',
		    data:{'user':<?php echo $to_customer['id'];?>,'content':obj,"Channel_id":Channel_id,'msg_type':msg_type,"tribe_id":'<?php echo $tribe_id;?>'},
		    beforeSend:function()
      		{ 
...
function sendImg(blob,parms,send){
	...
	$.ajax({ 
	    ...
	    type:'post',
	    dataType:'json',
	    data:{'user':<?php echo $to_customer['id'];?>,'blob':blob,"Channel_id":Channel_id,"tribe_id":'<?php echo $tribe_id;?>'},
	    beforeSend:function()
  		{ 
	    	var myDate = new Date();//获取系统当前时间
	    	var stamp = Date.parse(myDate);
...
function  selectImg(obj){
    ...
    if(files.length == 1){
    	var fileImg = files[0];
        imgUrl = window.URL.createObjectURL(fileImg);
...
<script type="text/javascript">
  ...
  // $('.personal_chat_list').css('min-height',(window_height - 130));

    // $('.personal_chat_box').on('touchstart',function(){
    //    $('#form_article').blur();
    //    $('.foot_bottom').css('position','fixed');
    // }); 
