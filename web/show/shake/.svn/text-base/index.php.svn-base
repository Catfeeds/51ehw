 ...
 success: function(json){
  doit();
       function doit(){
        ...
        content:'<p><img class="ui avatar image" src=<?php echo Web_ROOT;?>'+json[i]['avatar']+'><xb>'+json[i]['phone']+'</xb></p>',
       ...
       $("#ranking div:eq("+i+")").children('span').css({"width":scrwidth,"visibility":"visible"});
		for(i=0;i<<?php echo $xuanzezu[8];?>;i++){
			$("#ranking div:eq("+i+")").children('span').html('<p><img class="ui avatar image" src=<?php echo Web_ROOT;?>'+json[i]['avatar']+'><xb>'+json[i]['phone']+'</xb></p>');
			}
        isstop = 1;
        }
...
<div id="ranking" class="ui page grid">
   ...
   <div class='progress-bar  <?php if($i < 3) echo $class[$i] ;?>'><su><?php echo $i + 1 ; ?></su><su2></su2>
...
<div id="bignum" class="ui page grid">
  ...
  <img src="../wall/images/ma.jpg"></center>
