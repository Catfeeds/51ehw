...
{ 
	...
	$.ajax({ 
		url:'<?php echo site_url('Circles/Add_Comment')?>',
		type:'post',
		dataType:'json',
		data:{'obj_id':obj_id,'content':content,'tribe_id':<?php echo $tribe_id?>
		<?php if( $to_customer_id && $parent_id ){?>
		,'to_customer_id':'<?php echo $to_customer_id?>','parent_id':<?php echo $parent_id?>
		<?php }?>
		...
		success:function (data)
		{
			...
			if( data.status )	
			{
				...
				$('#back').val(1);
				window.setTimeout("window.location.href='<?php echo site_url('Circles/index/'.$obj_id.'?tribe_id='.$tribe_id)?>'", 1000);
        		return;
			}
