...
class Resource extends Front_Controller
{
    ...
    public function resource_list()
    {
        ...
        //删除session中未保存图片
        $certificate = FCPATH . $this->session->userdata("certificate");
        $logo = FCPATH . $this->session->userdata("logo");
        $ce = FCPATH . $this->session->userdata("ce");
        $certificate!=FCPATH&&file_exists($certificate)?unlink($certificate):'';
        $logo!=FCPATH&&file_exists($logo)?unlink($logo):'';
        $ce!=FCPATH&&file_exists($ce)?unlink($ce):'';
