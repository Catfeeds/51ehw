...
class Corporation_mdl extends CI_Model {
	...
	public function search_shop( $search_shop='',$app_id=null ){
	    ...
	    if( $search_shop) {
	        $search_shop = trim($search_shop," ");	        
	        $search_shop = preg_replace("/[\s]+/is"," ",$search_shop);
...
// 	    echo $this->db->last_query();
	 ...
	 * 根据条件搜索用户=企业的信息。
	 */
	public function Customer_Corp_Info( $sift = array() )
	{ 
	    if( empty( $sift['where']['name'] ) )
	        return false;
	    
	    $this->db->select('c.name,c.mobile,c.nick_name,cc.*');
	    $this->db->from('customer as c');
	    $this->db->join('customer_corporation as cc','cc.customer_id = c.id');
	    $this->db->where('c.name',$sift['where']['name'] );
	    $query = $this->db->get();
// 	    echo $this->db->last_query();
	    return $query->row_array();
	}
