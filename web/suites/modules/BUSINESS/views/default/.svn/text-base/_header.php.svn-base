...
//      shield.style.width = "100%";  
     ...
     strHtml += " <li style=\"background:#72c312;width:300px;text-align:center;font-weight:bold;height:40px;line-height:40px;margin:0px auto\"><input style=\"width:300px;background:none;line-height:40px;border:none;color:#fff;font-size:16px; cursor:pointer\" type=\"button\" value=\"确 定\" onclick=\"doOk()\" /></li>\n";  
     strHtml += "</ul>\n";  
	 strHtml += "</div>\n";  
	...
	<div class="eh_toolbar clearfix">
		<div class="fl">
			...
			<!--登录状态 开始-->
				<li>您好，<span><a href="<?php echo site_url('member/info') ?>"><?php echo $this->session->userdata('user_name');?> </a></span><span><a
						...
						<li class="line">|</li>                    
                <li class="line_xia"><a href="<?php echo site_url('Member/Message');?>"><b class="icon-mail"></b><span id="youjian"> </span></a></li>
				<!--登录状态 结束-->
                                    <?php }?>
		...
		<div class="fr">
			<ul class="sf-menu" id="sf-menu">
				...
				<?php }else if($this->session->userdata('corporation_id') > 0 && $this->session->userdata('corporation_status') == 1 ){?>
				<li><a href="<?php echo site_url("member/info");?>">个人中心</a></li>
				<li class="line">|</li>
				...
				<li class="line">|</li>
				<li>
				<a href="<?php echo site_url("corporation/home_page");?>">商家入驻</a>
                </li>
               <!-- <li class="line">|</li>-->
                <li>
				...
				<?php }?>
				<?php if($this->session->userdata ( 'tribe_manager' )){?>
				    <li class="line">|</li>
				    	<li><a href="javascript:Tribelist(0);">部落管理</a></li>
				<?php }?>
                <li class="line">|</li>
    ...
    <div class="fenzhan">
      ...
      <div class="con_w">
        ...
        <div class="con_fz">
          ...
          <div class="hot">
            <!--
             <span><a href="/hangzhou/">
              杭州
              </a></span>  <span><a href="/shuyang/">
              沭阳
              </a></span>  <span><a href="/beijing/">
              北京
              </a></span>  <span><a href="/shanghai/">
              上海
              </a></span>  <span><a href="/guangzhou/">
              广州
              </a></span>  <span><a href="/tianjin/">
              天津
              </a></span>  <span><a href="/shenyang/">
              沈阳
              </a></span>  <span><a href="/shenzhen/">
              深圳
              </a></span>-->
          </div>
          <div class="search1">
...
<div class="alert_bg_header crop_table" hidden>
...
<?php };?>
<?php if($this->session->userdata ( 'tribe_manager' )){ ?>
    <div class="alert_bg_header tribe_table" hidden>
    <div class="alert_header" >
        <span class="cha">x</span>
        <h5>请选择部落<a href="javascript:close();"  ></a></h5>  
        <div class="alert_er_header">
            <ul id="tribe_boxs">
            </ul>
            <a href="javascript:SureTribe();"><h4>确认</h4></a>
        </div>
    </div>
</div>
<?php }?>
...
$(document).ready(function(){
	...
	<?php if( $this->session->userdata('user_in') ):?>
        ...
        $.ajax({
            url:'<?php echo site_url('Member/Message/Not_Read_Message')?>',
            type:'get',
            data:{},
            dataType:'json',
            success:function( data )
            {
                 if( data.status && data.data.not_read != 0 )
                 {
                     
                      $('#youjian').append('(<i id="head_not_read" style="color:red;">'+data.data.not_read+'</i>)');
                 } 
             },
            error:function(){}
        })
...
$(function(){
	...
	function Tribelist(){
		$.ajax({
			url:'<?php echo site_url("Tribe/ManagementTribe");?>',
			dataType:'json',
// 			async:false, 
			type:'get',
			data:{},
				 ...
				 if(data.list.length>0){
					 ...
					 for(var i=0;i<data.list.length;i++){
						 	if(data.list[i]['status'] == 2){
    							 result += '<li onclick="alert_er_header(this);">'; 
    							 result += '<a href="javascript:vido(0);" >'+data.list[i]['name']+'</a>';
    							 result += '</li>';
    							 result += '<input type="hidden" value="'+data.list[i]['id']+'">';
						 	}else{
							 	
						 		//  result += '<li style="background:unset;">'; 
         //                         // result += '<a href="javascript:void(0);">'+data.list[i]['name']+'<span style="float: right;">'+(data.list[i]['status']==1?"审核中":"审核不通过")+'</span></a>';
								 // result += '<a href="javascript:void(0);">'+data.list[i]['name']+'<span style="float: right;">'+(data.list[i]['status']==1?"审核中":"审核不通过")+'</span></a>';
								 // result += '</li>';
								 // result += '<input type="hidden" value="'+data.list[i]['id']+'">';
						 	}
					 }
					 $('#tribe_boxs').html(result);
	...
	/*点击切换*/
	 function alert_er_header(obj){
	 	  $(obj).addClass("alert_er_dianj").siblings().removeClass("alert_er_dianj");	
		  $(obj).addClass("tribe_select").siblings().removeClass("tribe_select");	  
	 }
	 
	//切换部落
	function SureTribe(){
        var tribe_id = $(".alert_er_dianj").next().val();//店铺id
        ...
        location.href="<?php echo site_url("tribe/managingtribes");?>/"+tribe_id;
...
// 			async:false, 
			...
			success:function(data){
				 if(data.corplist.length>0){
					 ...
					 for(var i=0;i<data.corplist.length;i++){
						 if(i == 0){
							 result += '<li class="alert_er_dianj corp_select">';
						 }else{
							 result += '<li>'; 
						 }
					 ...
					 $(".alert_er_header li").on("click",function(){
					 	  $(this).addClass("alert_er_dianj corp_select").siblings().removeClass("alert_er_dianj corp_select");
				 	  })
			 	 	 $(".crop_table").show();
		 		}
			},
	...
	//切换店铺
	function SureCorp(){
        var corp_id = $(".corp_select").next().val();//店铺id
        var status = $(".corp_select").next().next().val();//状态
        //隐藏弹窗
      	 $(".crop_table").hide();
    	 $('body').css({ 
                 "overflow-x":"auto",
