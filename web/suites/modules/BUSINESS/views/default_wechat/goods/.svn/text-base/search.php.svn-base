...
<div class="search-header" name="" style="background-color: #1A1A1A;height:50px;width:100%;position:fixed;top:0;left:0;">
   ...
   <form action="<?php echo site_url('search/wechat_search_goods') ?>" method="get" id="form_search" >
       <div class="nav_search" style="padding-top: 10px;margin-left:0px;">
          ...
          <input type="hidden" value="<?php echo !empty($cate_id)?$cate_id:0;?>" name="cate_id">   
       </div>
   </form>
</div>
<!--头部 end-->   
...
var cate_id = "<?php echo empty($cate_id)? '':$cate_id;?>";
var tribe_id = "<?php echo empty($tribe_id)? '':$tribe_id;?>";

var types = 10;//排序类型
var page = 1;//默认加载页数
<?php if(!isset($hot_keywords)){?>
dropload = $('#sort').dropload({
	...
	loadDownFn : function(me){
	    ...
	    // 加载数据
        var result = "";
        $.post("<?php echo site_url("search/ajax_search_goods");?>",{keyword:keyword,cate_id:cate_id,type:types,page:page,Latitude:Latitude,Longitude:Longitude,tribe_id:tribe_id},function(data){
            ...
            if(data["productList"].length>0){
                ...
                for(var i=0;i<data["productList"].length;i++){
                	...
                	//如果是共享服务部显示部落价 共享服务分类104164
                    if(data["productList"][i]['cat_id'] !=104164){
                 	   result += '<p style="padding-top: 20px">'+formatCurrency(data["productList"][i]['vip_price'])+'</p>';
                    }
                	if(types==5 || types==6){
                		var num  = data["productList"][i]['distance']/1000;
                    	if(num > 1){
                  		    result += '<p class="min">'+parseFloat(num).toFixed(1)+'Km</p>';
                    	}else{
               		        result += '<p class="min">'+parseFloat(data["productList"][i]['distance']).toFixed(1)+'m</p>';
                    	}
                	}

                	//判断商品是否属于我加入的部落
                	array_id = new Array();
                    if(data["productList"][i]['tribe_id']){
                    	var array_id = data["productList"][i]['tribe_id'].split(',');
                    }
                	if(user_in_tribe.intersect(array_id)[0]){
                    result += '<p class="tribe_money">部落价: '+formatCurrency(data["productList"][i]['tribe_price'])+'</p></em></li></a>';
                	}
