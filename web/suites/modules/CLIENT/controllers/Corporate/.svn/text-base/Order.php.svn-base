...
// 		echo $cate;
		...
		$data['branch_info'] = $this->branch->get_corp_branch($corporation_id);
...
// 		$config ['num_tag_close'] = '';
		...
		for ($i = 0; $i < 16; ++$i){  //0-14为status  15为corporation_branch_id
		    if($i == 15){
		        $data['status_'.$i] = $this->order_mdl->count_corporate_orders ( $corporation_id, array('conditions' => array('o.order_type' => $i)) );
