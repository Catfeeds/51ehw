...
<style type="text/css">
	...
	.commodity_record {margin-bottom: 50px;}
	.client_commodity_block_nav ul li {width: 33%;}
</style>
...
<!--               redirect("corporate/card_package/obtain_package_detail/{$id}/{$sender_id}/{$unix}");//已经领取完成直接跳转 -->
              <a href="<?php echo site_url("search/discount_goods/".$v["id"]);?>" class="client_commodity_record_send custom_border">立即使用</a>
              <?php };?>
              <?php }else{;?>
              <script type="text/javascript">
              function time_message<?php echo $v["id"];?>(){
         		 alert("此货包未到使用时间！请在<?php echo $v['coupon_start_at']?> 00:00:00 至<?php echo $v['coupon_end_at'];?> 24:00:00 期间使用！");
         		 return;
         	}
              </script>
              <a href="javascript:void(0);" onclick="time_message<?php echo $v["id"];?>();" class="client_commodity_record_send custom_border">立即使用</a>
              <?php };?>
