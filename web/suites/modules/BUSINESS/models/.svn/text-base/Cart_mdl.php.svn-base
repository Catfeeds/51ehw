...
class Cart_mdl extends CI_Model {
	...
	function reinit($customer_id) {	
		...
		foreach ( $carts as $cart ) {
            ...
            if (isset($cachecart[$cart['product_id'] . '_' . $cart['sku_id']])) {
                ...
                $qty = ($cart_session[$row_id]['qty']) + (int)($cart['quantity']);
                $data = array(
                    'rowid'  => $row_id,
                    'qty'    => $qty
                );
            ...
            }else {// 否则直接添加新商品到cookie购物车

                //保存ci购物车
                $data = array(
                    'id' => $cart['product_id'] . '_' . $cart['sku_id'],
                    'qty' => $cart['quantity'],//购买数量
                    'price' => $cart['vip_price'],
                    'product_id'=> $cart['product_id'],
                    'name' => $cart['product_name'],
                    'sku_id' => $cart['sku_id'],
                    'corporation_name' => $cart['corporation_name'],
                    'status' => 1,//状态：0无效1正常2库存不足
                    'stock' => $cart["stock"],//商品库存
                    'sku' => (!empty($cart["sku"])?$cart["sku"]:""),
                    'corporation_id' => $cart['corporation_id'],
                    'options'=>array("goods_img"=>$cart['img_goods'])
                );
        ...
        foreach ($this->cart->contents() as $items) {
            $cart = array(
                ...
                'quantity' => $items["qty"],//购买数量
                'vip_price' => $items["price"],
                'product_name' => $items['name'],
                'sku_id' => $items["sku_id"],
                'corporation_name'=>$items["corporation_name"],
                'sku' => (!empty($items["sku"])?$items["sku"]:""),
                'stock' => $items["stock"],
     ...
     * @param int $sku_id
     */
	public function deleteCart($customerid,$product_id,$sku_id)
	{
    ...
    //App
    public function deleteAPPCart($id,$customerid){
        $this->db->where("id",$id);
        $this->db->where("customer_id",$customerid);
        $this->db->from("cart");
        $cart = $this->db->get()->row_array();
        if($cart){
            $this->db->delete('cart', array(
                'id' => $id,
                'customer_id' => $customerid,
	 ...
	 * @param int $qty
	 */
	public function updateCart($customerid,$product_id,$sku_id,$data)
	{
        $this->db->where(array(
            'customer_id' => $customerid,
            'product_id' => $product_id,
            ...
            foreach ($cachecart as $key => $cc) {
                if ($cc["product_id"] == $cart["product_id"] && $cc["sku_id"] == $cart["sku_id"]) {
                    ...
                    $data = array(
                        'quantity' => $quantity,
                        'price' => $cart['price'],
                        'freight' => $this->freight_count($productInfo[$cart["product_id"]], $quantity)
                    );
                    $this->db->set($data);
                    $this->db->where(array(
                        'id' => $cart["id"],
        ...
        foreach ($cachecart as $key => $cc) {
            ...
            if (! $flag) {
                $data = array(
                    ...
                    'freight' => $this->freight_count($productInfo[$cc["product_id"]], $cc['quantity'])
                );
