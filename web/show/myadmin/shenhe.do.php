...
function zengjian()
{
    ...
    if ($cid == "2") {
        ...
        if ($cid == "1") {
            ...
            $sql = "INSERT INTO `weixin_vote` (`id`,`name`,`res`) VALUES ('','极智大屏幕','0')";
			$db->query($sql);
            //mysql_query($sql);
...
{
	$db = mysql::getInstance();
	$db->query("TRUNCATE TABLE weixin_wall");
	$db->query("UPDATE `weixin_wall_num` SET `num` = 1");
...
{
	$db = mysql::getInstance();
	$db->query("TRUNCATE TABLE weixin_flag");
	$db->query("UPDATE `weixin_cookie` SET `cookie` =  '',`cookies` = '',`token` = '' WHERE `id` = '1'");
	$db->query("UPDATE `weixin_wall_num` SET `lastmessageid` = 0");
...
function shake_ready()
{
	...
	$db = mysql::getInstance();
	$db->query("UPDATE `weixin_wall_config` SET `isopen` = '1'");
	$db->query("UPDATE `weixin_shake_toshake` SET `point` = '0'");
...
function shake_reset()
{
	...
	$db = mysql::getInstance();
	$db->query("TRUNCATE TABLE weixin_shake_toshake");
	$db->query("UPDATE `weixin_wall_config` SET `isopen` = '1'");
