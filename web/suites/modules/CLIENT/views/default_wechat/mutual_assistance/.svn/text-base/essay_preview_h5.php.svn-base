...
<div class="essay_preview">
	...
	<div class="essay_preview_nav">
		<ul>
		    <li style="width: 40%;" class="<?php if($type =='all'){echo 'essay_active';}?> essay_classify"><a href="javascript:void(0);">商品分类<span class="icon-xiala icon-fold essay_icon"></span></a><span class="pl14">|</span></li>
		    <li style="width: 30%;" class="<?php if($type =='new'){echo 'essay_active';}?> essay_issue"><a href="javascript:void(0);" onclick="navigation(1);">最新发布</a><span class="pl14">|</span></li>
		    <li style="width: 30%;" class="<?php if($type =='sale'){echo 'essay_active';}?> essay_sales"><a href="javascript:void(0);" onclick="navigation(2);">销量</a></li>
		</ul>
	</div>
    	        ...
    	        $.post("<?php echo site_url("shop/ajax_shop_pro_list");?>",{page:page,order:type,cate_id:cate_id,isparent:isparent,shopid:<?php echo $shop['id']?>},function(data){
    	            if(data.produtList.length>0){
    	                for(var i=0;i<data.produtList.length;i++){
    	                	...
    	                	var url = "<?php echo site_url('goods/detail').'/'?>"+data['produtList'][i]['id'];
    	                	result += '<a href="'+url+'">';
...
//分享
function share(){
	 window.location.href="<#share_shop#>name=<?php echo $shop['name'];?>的互助店@desc=刚刚在51乐赚看到一间不错的店铺，好东西要一起分享，快来看看吧!@url=<?php echo site_url('shop/skipping')."?parent=".$parent."&mark=".base64_encode(4);?>";
}
<?php }?>
