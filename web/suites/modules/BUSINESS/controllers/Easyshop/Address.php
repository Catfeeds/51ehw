...
class Address extends Front_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model('easyshop_address_mdl', 'address');
    }
    ...
    public function index( $is_order_check = 0)
    {
        ...
        $this->load->view('easyshop/address/address', $data);
    ...
    public function add($status = 0)
    {
        if ($status && isset($_SERVER['HTTP_REFERER'])) {
            ...
            $this->session->set_userdata('ref_from_url', site_url('easyshop/address/address'));
        }
    ...
    public function edit($id = 0, $path = "")
    {
        ...
        if ($id) {
            ...
            if ($data['address']) {
                ...
                $this->load->view('head', $data);
                $this->load->view('_header', $data);
                $this->load->view('easyshop/address/address_edit', $data);
                $this->load->view('_footer', $data);
                $this->load->view('foot', $data);
            } else {
                redirect('easyshop/address');
            }
        } else {
        ...
        if ($address_id) {
            ...
            if ($this->address->update($address_id, $customer_id)) {
                if ($back_path != "") {
                    ...
                    redirect('easyshop/address/index/2');
                }
            }
        } else {
