...
class Fav extends Front_Controller {
	...
	public function ajax_add()
	{
		...
		//检查商品是否存在
		$product = $this->goods_mdl->load($pid);
        if(!$product){
            $data = array("status"=>1,"err"=>"商品不存在");
            echo json_encode($data);exit;
        }

		
		if($this->favourites_mdl->_check_fav($customer_id,$pid))
		{
		    //取消收藏
			if($this->favourites_mdl->deletefav($product['id'],$customer_id)){
		          $data["status"] = 2;
		    }else{
		          $data["status"] = 3;
		    }
			...
			//添加收藏
			if($this->favourites_mdl->add_fav($item))
			{
				$data["status"] = 4;
			}else{
				$data["status"] = 5;
	...
	public function ajax_check()
	{
        ...
        if($this->favourites_mdl->_check_fav($customer_id,$id))
        {
			echo 'true';
        }else{
            echo 'false';
	...
	public function store_corporation()
    {
        ...
        $customer_id = $this->session->userdata('user_id');
        $this->load->model('goods_mdl');
        if ($this->goods_mdl->check($customer_id, $corp_id)) {
            echo '该店铺您已经收藏了';
        } else {
            if ($this->goods_mdl->add_corporation($customer_id, $corp_id)) {
                echo "收藏成功";
            } else {
                echo "收藏失败";
            }
    ...
    public function batch_add (){
        ...
        //查询是否收藏过
        $fav_exists = $this->favourites_mdl->_check_fav($customer_id,$id);
        //去除收藏过的商品
        ...
        if($pid){
            ...
            $row = $this->favourites_mdl->add_fav($data,$customer_id);
            if($row){
                echo 'ok';
            }else{
                echo 'fail';
