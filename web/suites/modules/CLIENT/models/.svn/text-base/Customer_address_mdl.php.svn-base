...
class Customer_address_mdl extends CI_Model
{
    ...
    {
        $this->db->select("a.*,concat(b.region_name,'省',c.region_name,'市',d.region_name) as address_for_name");
        $this->db->from("customer_address as a");
        $this->db->join("region as b","a.province_id=b.region_id",'left');
        $this->db->join("region as c","a.city_id=c.region_id",'left');
        $this->db->join("region as d","a.district_id=d.region_id",'left');
