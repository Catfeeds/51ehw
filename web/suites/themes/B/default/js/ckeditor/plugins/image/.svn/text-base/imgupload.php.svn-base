...
if(isset($_FILES['upload']) && strlen($_FILES['upload']['name']) > 1) {
  ...
  $uploadpath = $_SERVER['DOCUMENT_ROOT']. "/".$upload_dir . $newImg_name;       // full file path
 ...
 echo "<script>alert('ggg".$_SESSION["app_info"]["id"]."');</script>";
  if($err == '') {
    if(move_uploaded_file($_FILES['upload']['tmp_name'], $uploadpath)) {
      $CKEditorFuncNum = $_GET['CKEditorFuncNum'];
      $url = $site. $upload_dir .$newImg_name; //$img_name;
