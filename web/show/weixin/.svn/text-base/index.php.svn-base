...
$items = array(
	    ...
	    new NewsResponseItem("头像，昵称，性别等个人信息"),
	   new NewsResponseItem("您的验证码为：{$veryfs}"),
new NewsResponseItem("请您回复验证码参与！")
        );
    ...
    protected function onText()
    {
        ...
        if ($xuanzezu[18]) {
            if ($content == '退出') {
                ...
                $reply = array(
                    new NewsResponseItem('您已成功退出本活动系统。', '')
                );
                $check[2] = '-10';
            }
        }
        ...
        if ($check[2] == '1') {
            ...
            //error_log("验证码：".$content."ddd:".$veryfs);
            // 判断验证码正确
            if ($content == $veryfs) {
    ...
    protected function monigetinfo($from, $time, $content, $veryfs)
    {
		...
		error_log($isluru."RRRR");
        if ($isluru == 1) {
            $reply = array(
                new NewsResponseItem("获取头像失败"),
                new NewsResponseItem("微信端服务器繁忙，您的信息获取失败，请重新回复【{$veryfs}】！")
            );
        } else {
    ...
    protected function writeinwall($content, $time, $q, $type = '')
    {
        ...
        if ($xuanzezu[9] == 0) {
			...
			$row = $db->getRow('select id from `weixin_wall` WHERE id =(select max(id) from `weixin_wall`)',MYSQL_ASSOC);

            //$cid = mysqli_query($this->dblink, 'select id from `weixin_wall` WHERE id =(select max(id) from `weixin_wall`)');
...
$wechat = new MyWechat('ninthleaf', false);
$wechat->run();
