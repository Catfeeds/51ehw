...
class Home extends Front_Controller
{
    ...
    public function index()
    {
        ...
        $this->session->unset_userdata("labe_foot_nav_color");//重置商会颜色、
        ...
        if(stristr($_SERVER['HTTP_USER_AGENT'],"Android") || stristr($_SERVER['HTTP_USER_AGENT'],"iPhone") || stristr($_SERVER['HTTP_USER_AGENT'],"wp")){
            ...
            $data['storey_list'] = $this->getHomeTheme();
        }
        else {
        ...
        // 首页获取需求banner图
//         $this->load->model('demand_mdl');
//         $demand_logo = $this->demand_mdl->get_logo();
     
        $data['corp_status'] =true;
//         $data['user_in'] = false;
//         if($user_id){
//             $data['user_in'] = true;
...
//             if($corp){
//                 $data['corp_status'] =true;
//                 //'抱歉，您需申请成为商家用户才能查看需求信息，有疑问请联系平台客服'
//             }
//         }
        ...
        if(isset($user_id) && $user_id){
            //查询判断是否部落管理者
            $tribe = $this->tribe_mdl->ManagementTribe($user_id);
            if($tribe){
                $this->session->set_userdata("flag",true);
            }
            
            $data['Msg_status'] = $this->Message->check_read($user_id);
           
            if(!$data['Msg_status']){
                $NotReadNum = $this->getChatNotReadNum();
                $data['Msg_status'] = $NotReadNum;
            }
        }else{
            $data['Msg_status'] = false;
        }
        $data["flag"] = !empty($flag);
        $data['head_set'] = 5;
		$data['foot_icon'] = 1;
		$data['Home'] = true;
    ...
    public function getHomeTheme(){
      ...
      //判断是测试环境还是正式环境
        ...
        if($demand){
            if($setting){
                $link = 'index.php/_BUSINESS/';//正式上有分站点区分不能用域名进行判断(域名不能固定死)
                if(!is_numeric(strpos($demand['link_path'], $link))){
                    $demand['link_path'] = base_url().$link.$demand['link_path'];
                }else{
                    if( !is_numeric(strpos($demand['link_path'], base_url()))){
                        $demand['link_path'] = base_url().strstr($demand['link_path'], 'index.php');
                    }
                }
                $demand['img_path'] = IMAGE_URL.$demand['img_path'];
            }else{
                $link = 'http://www.test51ehw.com/index.php/_BUSINESS/';
                if(!is_numeric(strpos($demand['link_path'], $link))){//需要处理比较0和false之间的区别
                    $demand['link_path'] = $link.$demand['link_path'];
                }
                $demand['img_path'] = 'http://www.test51ehw.com/uploads/B/'.$demand['img_path'];
            }
            
            $data_info['demand'] = $demand;
        }
        ...
        foreach ($list as $key =>$val){
            ...
            if($val['level_name'] == '中间导航' && empty($val['img_path']) || $val['level_name'] == '中间导航' && empty($val['link_path'])){
                    unset($list[$key]);
            }else{
                if($setting){
                    //$link = 'http://www.51ehw.com/index.php/_BUSINESS/';
                    $link = 'index.php/_BUSINESS/';//正式上有分站点区分不能用域名进行判断(域名不能固定死)
                    if(!is_numeric(strpos($val['link_path'], $link))){
                        $list[$key]['link_path'] = base_url().$link.$val['link_path'];
                    }else{
                        if( !is_numeric(strpos($val['link_path'], base_url()))){
                            $list[$key]['link_path'] = base_url().strstr($list[$key]['link_path'], 'index.php');
                        }
                    }
                    $list[$key]['img_path'] = IMAGE_URL.$val['img_path'];
                    
                }else{
                    $link = 'http://www.test51ehw.com/index.php/_BUSINESS/';
                    if(!is_numeric(strpos($val['link_path'], $link))){//需要处理比较0和false之间的区别
                        $list[$key]['link_path'] = $link.$val['link_path'];
                    }
                    $list[$key]['img_path'] = 'http://www.test51ehw.com/uploads/B/'.$val['img_path'];
                }
...
//             }
            ...
            if ($data["corporation"]['template_url'] != "") {
                ...
                $produtList = $this->goods_mdl->get_lists_with_condition($data["pagesize"], $offset, $cateid, array(), $options, 'p.id,p.name,vip_price,goods_thumb', null, $app_id, $customer_id, $section_id, $corporationid);
    ...
    public function GetShopGoods($corporation_id)
    {
        ...
        $status_message = array(0=>'您还没有开店，请联系客服协助 400-0029-777',2=>'店铺已被冻结');
        $approval_status_message = array(0=>'没有企业',1=>'企业审核中',3=>'企业审核不通过');
        
        // 店铺基本信息
        $data["corporation"] = $this->customer_corporation_mdl->getById($corporation_id);
       
        if( array_key_exists( (int) $data["corporation"]['status'], $status_message) )
        { 
            echo "<script>history.back(-1);alert('{$status_message[$data["corporation"]['status']]}');</script>";exit;
        }
        
        if( array_key_exists( (int) $data["corporation"]['approval_status'], $approval_status_message) )
        {
            
            echo "<script>history.back(-1);alert('{$approval_status_message[$data["corporation"]['approval_status']]}');</script>";exit;
        }
        ...
        if($is_ajax){
            $data['productList'] = $this->goods_mdl->get_lists_with_condition($page, $offset, 0, array(), $options, 'p.id,p.name,vip_price,goods_thumb', null, $app_Info['id'], 0, 0, $corporation_id);
        } else {
            $data['productList'] = $this->goods_mdl->get_lists_with_condition($page, 0, 0, array(), $options, 'p.id,p.name,vip_price,goods_thumb', null, $app_Info['id'], 0, 0, $corporation_id);
        }
