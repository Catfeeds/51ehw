...
if($shake)
 {
echo "if(event.keyCode == 89){window.open('".Web_ROOT."/shake/');}";
	}
?>
...
var mtime;
var data=new Array();
data[0]=new Array('0','/wall/images/ma.jpg','51易货网','欢迎来到微信上墙，发送图片也可以上墙哦！','');
//var word_id='96';
<?php
...
function messageAdd()
{
	...
	//alert(cur);
	//alert(data[cur]);
    if (data[cur].length > 4 && data[cur][4] == ''){
        var str='<li id=li'+cur+' onclick="viewOne('+cur+',this);"><div class=m1><div class=m2><div class="pic"><img class="circular ui image" src="<?php echo Web_ROOT;?>'+data[cur][1]+'" width="100" height="100" /></div><div class="c f2"><span>'+data[cur][2]+'</span><span>：</span><word>'+data[cur][3]+'</word></div></div></div></li>';
        }
        else {
    ...
    if (data[cur] && data[cur].length > 4 && data[cur][4] != ''){
		//alert("dfdfdfd");
		viewOne(cur,cur);
...
function messageData()
{
    ...
    var url='http://www.51ehw.com/show/wall/api.php';
    $.get(url,{"lastid":lastid},function(da) {
		if(da != null && da != "")
		{
			var d = JSON.parse(da);
			//alert(d['ret']);//return;
