...
//                     header("Location: " . $app_info['site_url'] ."_CLIENT/Goods/detail/$id");
//                 } else {
//                     show_404();
//                 }
//                 return;
...
//         echo $this->db->last_query();exit;
        ...
        if($shop){
            //分享连接参数配置
            $data['type'] = base64_encode(2);
            $data['parent'] = base64_encode($user_id);
            $exist = $this->shop->check_product($shop['id'],$id);//检查是否已添加
            if($exist){
                $data['communal'] = $exist['id'];
                if(base_url() == 'http://www.test51ehw.com/'){
                    $data['url'] = '<#share_product#>name='.$data['details']['name'].'@desc=我在善活精英上发现了一款不错的商品，赶快来看看吧!@logo='.base_url().'uploads/C/'.$data['details']['goods_thumb'].'@url='.site_url('shop/skipping')."?id=".$data['communal']."&type=".$data['type']."&parent=".$data['parent']."&mark=".base64_encode(3);
                }else{
                    $data['url'] = '<#share_product#>name='.$data['details']['name'].'@desc=我在善活精英上发现了一款不错的商品，赶快来看看吧!@logo='.IMAGE_URL.$data['details']['goods_thumb'].'@url='.site_url('shop/skipping')."?id=".$data['communal']."&type=".$data['type']."&parent=".$data['parent']."&mark=".base64_encode(3);
                }
               
            }else{
                $data['communal'] = NULL;
                $data['url'] = '<#share_product#>';
            }
        }else{
            $data['communal'] = NULL;
            $data['url'] = '<#share_product#>';
    ...
    public function notexist(){
        ...
        $this->load->view('goods/solid_out', $data);
        $this->load->view('_footer', $data);
        $this->load->view('foot', $data);
    }
    ...
    public function detailapp($id = 0, $app = "android", $userid = 0, $sessionid = 0 , $version = "")
    {
        ...
        if($shop){
            ...
            if($exist){
                $data['communal'] = $id;
                if(base_url() == 'http://www.test51ehw.com/'){
                    $data['url'] = '<#share_product#>name='.$data['details']['name'].'@desc=我在善活精英上发现了一款不错的商品，赶快来看看吧!@logo='.base_url().'uploads/C/'.$data['details']['goods_thumb'].'@url='.site_url('shop/skipping')."?id=".base64_encode($id)."&type=".$data['type']."&parent=".$data['parent']."&mark=".base64_encode(3);
                }else{
                    $data['url'] = '<#share_product#>name='.$data['details']['name'].'@desc=我在善活精英上发现了一款不错的商品，赶快来看看吧!@logo='.IMAGE_URL.$data['details']['goods_thumb'].'@url='.site_url('shop/skipping')."?id=".base64_encode($id)."&type=".$data['type']."&parent=".$data['parent']."&mark=".base64_encode(3);
                }
            }else{
                $data['communal'] = NULL;
                $data['url'] = '<#share_product#>';
        ...
        // 新版app 立即购买、加入购物车 函数提供link不同导致app奔溃，暂时提供app 版本使用不用详情页
//         if($version == 2.4){
