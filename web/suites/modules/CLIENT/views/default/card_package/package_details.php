...
.matou{ margin-top:20px; margin-bottom:20px;}
table{ text-align:center}
.gudong{overflow: hidden; overflow-y:auto ;overflow-x: hidden; max-height:540px;}
.tonhji-nei{ height:auto; line-height:29px;}
</style>
	...
	<div class="cmRight manage_new_cmRight  manage_a_cmRight manage_b_cmRight" style="text-align: left">
          ...
          <div class="dianpu_01_con01 clearfix" id="recite_add">
              ...
              <div class="dianpu_zhong">
                ...
                <div class="dianpu_01_right">
                	<ul>
                   <li class="dingwe"><span style="color: #fe4101"><?php echo $package['donation']?"可转赠其他用户":"禁止转赠"; ?></span></li>
                    </ul>
                </div>
              </div>
                
              </div>
             <?php if($package['donation']){;?>
             <div class="tongji-di">
               ...
               <table width="907" border="1" cellpadding="0" cellspacing="0" class=" manage_b_table1">
                    ...
                    <?php foreach ($accredit as $v){?>
                     <tr> 
                       ...
                       <th width="340px"><a onclick="cancel(this,'<?php echo $v['p_id']?>','<?php echo $v['customer_id']?>')"  style="color:#fe4101; text-decoration:underline">取消授权</a></th>
                     </tr> 
                    <?php };?>
                    <?php };?>
                    </tbody>
                ...
                <?php if(!$accredit){?>
                 <div class="result_null" style="margin-top:10px; color:#fe4101;">暂无授权用户</div>
                <?php };?>
                <div class="matou">
                  <div class="tongji" style="overflow:hidden">
                   <ul>
                    <li>统计信息:</li>
                   </ul>
                  <div class="tonhji-nei"><span>已领取：<?php echo $receivetotal;?></span><?php if($package["give_type"]==2){?><span>剩余未领取：<?php echo $number["not_number"];?></span><?php };?><p><span>已使用：<?php echo $use;?></span></p></div>
                  </div>
                  <h6>货包领取列表</h6>
                   <div class="gudong"> 
                <table width="907" border="1" cellpadding="0" cellspacing="0" class=" manage_b_table1">
                    <tbody>
                        <tr class="tr1 manage_b_tr1">
                            <th width="227px">用户</th>
                            <th width="340px">手机号码</th>
                            <th width="340px">使用状态</th>
                        </tr>
                        <?php if($situation){;?>
                        <?php foreach ($situation as $v){;?>
                        <tr> 
                        <th width="227px"><?php echo $v["wechat_nickname"]?$v["wechat_nickname"]:$v["nick_name"];?></th>
                        <th width="340px"><?php echo $v["mobile"];?></th>
                        <th width="340px"><span  style="color:#fe4101; ">
                        <?php if($v['status']==1){
                            echo "已使用";
                        }else{
                                echo "未使用";
                ...
                <?php if(!$situation){;?>
                <div class="result_null" style="margin-top:10px; color:#fe4101;">暂无用户领取</div>
                <?php };?>
              </div>
              </div> 
               </div>
