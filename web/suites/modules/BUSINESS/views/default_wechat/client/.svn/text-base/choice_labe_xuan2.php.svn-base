          ...
          <div id="loadmore">
              <?php if(count($Dictionary)>0){
                  foreach ($Dictionary as $k =>$v){ ?>
                  		<?php if($id){?>
                  		    <a href ="<?php echo site_url('member/requirement/add_Dictionary?id='.$v['id'].'&rid='.$id);?>"><span ><?php echo $v['name'];?></span></a>
                  		<?php }else{?>
                  		<a href ="<?php echo site_url('member/requirement/add_Dictionary?id='.$v['id']);?>"><span ><?php echo $v['name'];?></span></a>
...
function onsearch(limit){
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            var url = "<?php echo site_url('member/requirement/add_Dictionary?id=');?>";
            ...
            if( sorces.Dictionary.length>0){
                ...
                for(var i=0;i<sorces.Dictionary.length;i++){
                	html += '<a href ="'+url+sorces['Dictionary'][i]["id"]+'&rid<?php echo $id; ?>"><span >'+sorces['Dictionary'][i]["name"]+'</span></a>';
                    }
                <?php }else{?>
                for(var i=0;i<sorces.Dictionary.length;i++){
