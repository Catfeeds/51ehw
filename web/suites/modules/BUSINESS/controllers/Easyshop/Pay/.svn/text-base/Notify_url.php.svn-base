 ...
 * 
 */
class Notify_url extends NLF_Controller {
	 ...
	 * @return: 
	 */
	private function HandlingEvents()
	{ 
	    //查询充值单
	    $sift['where']['charge_no'] =  $this->chargeno;
	    $charge = $this->easy_charge_mdl->Load_by_Change_No( $sift );
        ...
        do{
            ...
            //更新主表状态+第三方单号。
            $set['charge_no'] = $this->chargeno;
            $set['status'] = 1;
            $set['payment_id'] = 1;
            
            if( !$this->easy_charge_mdl->Update( $set ) )
            { 
                //状态更新失败。
            ...
            {
                //订单支付--完成
                $this->load->model('easy_order_mdl');
                $result = $this->easy_order_mdl->AfterEasyOrder( $charge );
