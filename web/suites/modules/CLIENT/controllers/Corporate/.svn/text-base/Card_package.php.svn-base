...
class Card_package extends Front_Controller {
	...
	public function add_view($id=0) {
	    ...
	    $data["product"] = $this->product_mdl->find_products(array(), null, null,0,1);//店铺商品
...
//                 error_log("mkdir back:".mkdirsByPath($path));
     ...
     * @param int $p_id 卡包id
     */
    public function details($p_id=0,$status=0){
        //判断是否绑定手机
        if(!$this->is_binding){
            redirect('member/binding/binding_mobile');exit;
        }
        ...
        if($status==1){//已经领取查询的详情
            $customer_id = $this->session->userdata('user_id');
            $data['package'] = $this->card_package_mdl->receive($p_id,$customer_id,true);//查询详情
        }else{//未领取查询的详情
            $data['package'] = $this->card_package_mdl->get_card_package($p_id,true);//查询详情
        }
    ...
    function sure_verification($id=0){
        ...
        if(in_array($customer_id,array(6770,6765,4175,2933,5413,7335,4456,7267))){
