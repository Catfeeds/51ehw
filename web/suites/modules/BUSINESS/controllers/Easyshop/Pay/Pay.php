...
class Pay extends Front_Controller {
	...
	public function Index() 
	{
	    ...
	    $charge_info['order_source'] = 2;//$this->input->get_post('charge_source');//1:PC支付 2:H5支付 3:安卓 4:IOS 5后台
	    ...
	    //可选备注信息
	    $charge_remark[1] = '订单支付';
	   
	    
	    //如果不存在默认选择充值现金
	    if( empty( $charge_remark[$charge_info['charge_type']] ) )
	    { 
	       //404
	       return;
	    } 
	    
        $charge_info['remark'] = $charge_remark[$charge_info['charge_type']];
	    ...
	    do {
	        ...
	        $charge_info['chargeno'] = get_order_sn();
	       
	        if ( !$this->Charge_mdl->check_ordernum( $charge_info['chargeno'] )   )
	        {
	            $order_exist = false;
	        } 
	    ...
	    //生成支付信息 
	    $charge_id = $this->Charge_mdl->create( $charge_info );
...
// 	                redirect('Pay/Wechat_Qrcode_Pay/'.$charge_id);
	 ...
	 * 发起微信充值
	 */
	private function Pay( $charge_id )
	{ 
	    ...
	    $this->load->view ( 'easyshop/payment/wechay_pay', $data );
