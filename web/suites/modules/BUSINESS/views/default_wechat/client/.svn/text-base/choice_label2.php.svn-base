...
<div class="my-needs">
	...
	<div class="choice-label">
     ...
     <span class="fn-left choice-label-title2">选择行业</span>
	   ...
	   <?php if($type){ ?>
    	   <?php if(count($Dictionary)>0){?>
        	    <a href="<?php echo site_url('member/requirement/add_chioces?id='.$id)?>" class="model-select-text" data-value=""><?php echo $Dictionary['name'];?></a>
        	<?php }else{?>
        	 <a href="<?php echo site_url('member/requirement/add_chioces?id='.$id)?>" class="model-select-text" data-value="">请选择行业</a>
	   <?php } }else{?>
        <?php if(count($Dictionary)>0){?>
    	    <a href="<?php echo site_url('member/requirement/add_chioces')?>" class="model-select-text" data-value=""><?php echo $Dictionary['name'];?></a>
    	<?php }else{?>
    	 <a href="<?php echo site_url('member/requirement/add_chioces')?>" class="model-select-text" data-value="">请选择行业</a>
    	<?php } }?>
      </div>
	   </div>
	...
	<div class="maintenance-label-bt">
      <a href="javascript:sub();" class="maintenance-label-confirm">确认</a>
    </div>
...
$(".label-roundadd-icon").on("click",function(){
  ...
  $(".choice-fenlei").on("click",".model-select-option li",function() {
   ...
   $(".needs-list-shanchu").on("click",function(){
     ...
     $.ajax({
  		url:'<?php echo site_url("member/requirement/delconditions") ?>',
  	    dataType:'json',
  	    type:'get',
	     ...
	     $.ajax({
			    ...
			    success:function(data){
				    //分类返回空数组，即已经选择分类到最下级层或在一级分类中就已经没有下级分类了
			    	 if(data['parentlevel']){
				    	 //当返回数据中没有下级分类，通过返回的level 来判断是一级分类中就已经没有下级分类 还是最下级分类
				    	 if(data['parentlevel']==1){
					    	 //如果是一级分类就没有下级分类了则删除其余的标签
				    		 $('#fl1').parents('.border-none').nextAll().remove();
					    	 }else{
						    		//如果不是一级分类就没有下级分类了则不做改变
					    	 }
				    	 }else{
  ...
  function sub(){
	  ...
	  $.ajax({
		  url:'<?php echo site_url("member/requirement/add_Condition")?>',
		    dataType:'json',
		    type:'get',
		    data:{id:id,brand_id:brand,standard:standard},
		    success:function(data){
		    	window.location.href="<?php if($type=='edit'){echo site_url('member/requirement/editrequirements?id='.$id);}else{ echo site_url('member/requirement/addrequirements');}?>";
			    },
		    error:function(){
				alert("网络错误！");
					}
