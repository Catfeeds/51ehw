...
function submitOrder(obj,status) {
	...
	if ($('#address_id').val()) {
		...
		$.ajax({
			...
			type : 'post',
			data:{"product_data":product_data,"customer_remark":customer_remark,"address_id":address_id,"status":status,"package_id":package_id},
			dataType : "html",
			beforeSend:function(){     
			...
			success : function(data) {
				...
				case 'ok':// 成功
					...
					location.href = base_url + "/Member/order/all_order_pay/?order_id="+data.order_id;
//					$('#wrap_tanchuang_zhifu').show();
//					$('#all_order_price').text('M '+total_price);
//					$('#pay_').attr('onclick','all_order_pay("'+data.order_id+'")');
					break;
				case 'fail':// 订单生成失败执行
					alert('订单生成失败');
...
//					location.href = base_url + "/home";
				...
				$('#order_submit_status').css('background','#F9D40A');
