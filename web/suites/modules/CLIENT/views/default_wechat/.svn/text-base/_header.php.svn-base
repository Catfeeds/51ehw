...
//2. 分享接口
  ...
  document.querySelector('#onMenuShareAppMessage').onclick = function () {
	  ...
	  if('<?php echo $this->session->userdata("user_id");?>' == ""){
			location.href = '<?php echo site_url("customer/login");?>';
			return;
		}
	  alert("正在分享给朋友~！");
    wx.onMenuShareAppMessage({
      title: '<?php echo isset($title)?$title:"51易货网";;?>',
      desc: '<?php echo isset($title)?$title:"51易货网";;?>',
      link: '<?php echo current_url()."?parent_id=".$this->session->userdata("user_id");?>',
      imgUrl: '<?php echo base_url();?>/uploads/app/20150727182030.jpg',
