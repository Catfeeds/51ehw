...
// 		}
	 ...
	 * 加入购物车 和 立即购买公用方法
	 */
	public function add_goods($pid,$qty,$sku_id,$type="pc")
	{
        ...
        if($type == 'h5' ){
            $data = array(
                ...
                'options' => array(
                    ...
                    'special_price_start_at' => $product['is_special_price'] && $product['special_price_start_at'] <= date("Y-m-d H:i:s") && $product['special_price_end_at'] >= date("Y-m-d H:i:s") ? $product['special_price_start_at'] : '1970-01-01 08:00:00',
                    'special_price_end_at' => $product['is_special_price'] && $product['special_price_start_at'] <= date("Y-m-d H:i:s") && $product['special_price_end_at'] >= date("Y-m-d H:i:s") ? $product['special_price_end_at'] : '1970-01-01 08:00:00',
                ),
                'freight'=> $freight,
                'product_status' => 'ok'
            );
        ...
        //判断是否是h5立即购买
        if($type == 'h5'){
            $_carts = $this->cart->contents();
            foreach ($_carts as $k =>$v){
                $rowid = $v['rowid'];
            }
            $return =array('message'=>'ok', 'rowid'=>$rowid);
            return $return;
        }else{
