	...
	function create_links() {
		...
		if ($CI->config->item ( 'enable_query_strings' ) === TRUE or $this->page_query_string === TRUE) {
			...
			if ($CI->uri->segment ( $this->uri_segment ) != $base_page) {
				...
				// Prep the current page - no funny business!
				$this->cur_page = ( int ) $this->cur_page;
			} else if ($this->curr_page) {
				$this->cur_page = ($this->curr_page - 1) * $this->per_page;
			}
		}
		...
		if ($this->prev_link !== FALSE and $this->cur_page != 1) {
			...
			if ($i == 0 && $this->first_url != '') {
				...
				$output .= $this->prev_tag_open . '<a ' . $this->anchor_class . 'href="' . $this->base_url . $i . '" '.$this->prev_tag_css.'>' . $this->prev_link . '</a>' . $this->prev_tag_close;
			}
		}
		...
		if ($this->next_link !== FALSE and $this->cur_page < $num_pages) {
			...
			$output .= $this->next_tag_open . '<a ' . $this->anchor_class . 'href="' . $this->base_url . $this->prefix . $i . $this->suffix . '" '.$this->next_tag_css.'>' . $this->next_link . '</a>' . $this->next_tag_close;
		}
