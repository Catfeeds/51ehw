...
.order-charge-button {width: 95%;height: 45px;margin: 15px auto;background-color: #FDCF0C;border: 1px solid #FDCF0C; text-align: center;}
.order-charge-button a {display: block;text-align: center;font-size: 18px;height: 45px;line-height: 45px;}
...
.code-ball-main img {width: 75%;background: #fff;border: 5px solid #fff;margin-top: 10px;margin-bottom: 10px;}

/*核销员／时间*/
.verification {padding: 10px;border-bottom: 5px solid #E6E6E6;font-size: 14px;}

.order_text_box {margin-top: 10px;overflow: hidden;font-size: 14px;}
...
<div class="page clearfix">
		...
		<div class="order_list_title">
			<span class="fn-left " style="padding-right: 10px; font-size: 15px; padding-left: 10px;">
				<em class="icon-shop"></em></span><span style="font-size: 14px;">
					<?php echo isset($order['corporation_name'])?$order['corporation_name']:"&nbsp;&nbsp;&nbsp;";?>
				</span>
				<a href=""></a>
		</div>
		...
		<section style="padding-bottom: 0px;border-bottom: 1px solid #ccc;">
			<div class="good_list clearfix" >
				<div style="margin-bottom: 10px;">
					...
					<!-- 标题 -->
					<a href="<?php echo $v['product_id'] ? site_url('goods/detail/'.$v['product_id']) : 'javascript:;'?>" style="">
						<?php if($order['corporation_branch_id']){ ?>
						     <img src="images/default_biao.jpg" width="62" height="83" onerror="this.src='images/default_biao.jpg'" class="good_list_img">
						<?php  }else{?>
	   ...
	   <!-- 结算 start -->
		...
		<section style="padding-top: 0px; border-top: 5px solid #E6E6E6; border-bottom: 5px solid #E6E6E6; padding-bottom: 10px;">
		  ...
		  <?php }?>		
				...
				<?php if($order['total_product_price'] - ($order['total_price']-$order['auto_freight_fee']) > 0){?>
                <div style="border-bottom: 1px solid #E6E6E6; padding-bottom: 10px;">优惠
                  <span style="float: right;" class="c9"><span></span>
                  <span style="font-size: 12px !important;">－<?php echo $order['total_product_price'] - ( $order['total_price']-$order['auto_freight_fee'] );?></span>
                  <span style="padding-left: 5px;">货豆</span></span>
                </div>
                <?php };?>
                
                <?php if( $order['order_type'] == 1 ){?>
                <div class="order_text_box">货豆支付金额
			   ...
			   <?php }?>
                ...
                <div class="order_text_box">货豆支付金额
					<span style="float: right;">
						<span><?php echo number_format($order['total_price'] - $order['card_amount'],2,'.','') ?></span>
						<span style="font-size: 12px !important;"></span>
						<span class="c9" style="padding-left: 5px;">货豆</span>
					</span>
				</div>
			   ...
			   <?php if( $order['order_type']== 2 || $order['order_type'] == 3){ ?>
				<div class="order_text_box">储值卡支付金额
					<span style="float: right;">
						<span><?php echo $order['card_amount']?></span>
						<span style="font-size: 12px !important;"></span>
						<span class="c9" style="padding-left: 5px;">货豆</span>
					</span>
				</div>
				<div class="order_text_box">线上储值卡
					<span style="float: right;">
						<span><?php echo $order['savings_card_name']?></span>
						<span style="font-size: 12px !important;"></span>
					</span>
				</div>
				<?php }?>
				<?php if( $order['corporation_branch_id'] ){?>
                <div class="order_text_box anzhuo_xiaofei">消费分店
					<span style="float: right;">
						<span><?php echo $order['branch_name']?></span>
						<span style="font-size: 12px !important;"></span>
						<span class="c9" style="padding-left: 5px;"></span>
					</span>
				</div>
                <?php }?>
				<!-- 
                <div class="order_text_box">应支付金额
					<span style="float: right;">
						<span><?php echo $order['total_price'];?></span>
						<span style="font-size: 12px !important;"></span>
						<span class="c9" style="padding-left: 5px;">货豆</span>
   ...
   <!--   <section style="border-bottom: 5px solid #E6E6E6;" class="order-saomiao"> 
      <p style="font-size: 15px;">查看收货二维码<span style="float: right;font-size: 20px;" class="icon-erweima1"></span></p>
    </section>
    -->
    <!-- 二维码弹窗 
    <div class="code-ball order-detail-saomiao">
      <div class="code-ball-main">
          <span>此二维码仅供商家核销订单使用</span>
          <img src="images/order-saomiao.png" alt="">
    ...
    <p class="verification">核销员：xxx
      <span style="float: right;">核销时间：2016-07-25 12:12:12</span>
		...
		<?php else:
    		...
    		<!-- 备注 start -->
        	<p style="padding-top: 10px; padding-left: 10px; font-size: 15px; border-bottom: 35px solid #E6E6E6; padding-bottom: 10px;">备注：<?php echo $order['customer_remark'];?></p>
    		<!-- 备注 end -->
    		<?php 
    		endif;
		endif;?>
        ...
        <a href="<?php echo site_url('member/order/order_pay/'.$order['id'])?>" class="order_list_comment" id="pay_submit"
        	style="background: #fe4101 !important; border: none; margin-left: 10px; margin-right: 0;color:#fff;">去支付</a>
        ...
        if ($order['status'] == 5) : ?>
		<a href="<?php echo site_url();?>" class="pay-style gray-but c10"><i class=""></i>继续购物</a>
		<?php endif;
  ...
  <section style="display:none;">
    <div class="order-charge-button">
      <a href="javascript:void(0);">确认核销</a>
    </div>
  </section>
</div>
...
// alert("安卓手机");
 $(".anzhuo_xiaofei").css("margin-bottom","-10px");
...
<script type="text/javascript">
  // 点击查看收货二维码
  $(".order-saomiao").on("click",function(){
    $(".order-detail-saomiao").css("display","block");
  })
  $(".order-detail-saomiao").on("click",function(){
    $(".order-detail-saomiao").css("display","none");
  })
</script>
