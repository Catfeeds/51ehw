...
class Pay extends Front_Controller 
{
     ...
     * PC支付提交
     */
    public function charge_pay( $charge_id = 0, $pay_type = 'COM' )
    {
        ...
        $all_pay_type = array('COM','POR','COP');
        if( !in_array( $pay_type, $all_pay_type ) )
        { 
            echo '<meta charset="utf-8">
                <script type="text/javascript">
                    alert("支付产品不存在");
        ...
        $this->FrontConsume( $pay_type.$charge['chargeno'],$pay_date,$amount );
