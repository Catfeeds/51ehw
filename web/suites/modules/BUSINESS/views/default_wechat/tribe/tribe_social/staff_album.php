...
<div class="tribe_preson_show">
 ...
 <!--  <div class="tribe_preson_show_nav">
  ...
  <!-- 背景图片 -->
  <img id="bg_img" src="images/bg_tribe_01.png" alt="" onerror="this.src='images/bg_tribe_01.png'">
  <!-- 名字／职位 -->
  <div class="tribe_preson_show_name"><span><?php echo $member_name;?></span></div>
  <!-- 图片数量 -->
...
<div class="circle_add_apply_footer tribe_show_but01">
  <ul>
    <li style="background:rgba(214,182,135,0.8);"><a href="<?php echo site_url("Tribe_social/staff_upload/{$staff_id}");?>" class="corp_invite custom_button">帮他添加形象</a></li>
  </ul>
</div>
...
dropload = $('#album_box').dropload({
	  ...
	  loadDownFn : function(me){
		  var result = "";
		  $.post("<?php echo site_url("Tribe_social/loading_staff_albums");?>",{page:page,staff_id:'<?php echo $staff_id;?>'},function(data){
			  if(data['list'].length>0){
				  ...
				  <?php if(base_url() == 'http://www.test51ehw.com/' || base_url()=='http://localhost/51ehw/web/'){?>
            	    ...
            	    if( data['list'][i]['from_customer_id'] == <?php echo $from_customer;?>)
          			{
           	    	    result +='<a href="<?php echo site_url('Tribe_social/edit_staff_album');?>/'+data['list'][i]['photo_list'][0]['staff_album_id']+'" class="tribe_photo_edit" style="right:40px;">修改</a>';
                    	result +='<a href="javascript:void(0);" data-value ="'+data['list'][i]['photo_list'][0]['staff_album_id']+'" class="tribe_photo_edit del_btns">删除</a>';
          			}
	                  result +='</div>';
		                }
...
<script type="text/javascript">
  ...
  function delalbum(){
	$(".tuichu_ball").hide();
  	$.post("<?php echo site_url("Tribe_social/del_staff_album");?>",{staff_album_id:del_album_id},function(data){
  		if(data.status){
