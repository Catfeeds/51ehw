...
margin-top:0px;	
 ...
 /*.introduction_xia {background-color:#ffd600; color:#333333}*/
</style>
<?php }?>
...
<form action="<?php echo site_url('Circles/test');?> "method="post" enctype="multipart/form-data" id="form1">
  <div class="container container_topd">
       <div class="release_pictures">
         <div class="release_pictures_top">
         
         <textarea class="example_top" id="taContent" rows="3" name='content' placeholder="请输入帖子详情（1000字以内）"></textarea>
     ...
     <?php if( $role ){?>
         <input  type="hidden" name="is_top"  class="file" value="0" />
         <ul class="circle_ren_nei mt-10 qudiaoxiaxian" id="introduction_xia" >
         	<li><span class="zhong_lei">是否置顶</span> <span class="zhong_fig">  <input type="checkbox" id="checkbox_id" class="aui-switch"></span></li>
         </ul>
     <?php }?> 
...
{ 
  ...
  $("#test").empty();
	...
	$(".introduction_xia").html("发布中");
	 ...
	 var percent = $('.percent');
	$('#form1').ajaxSubmit({
  	    url: '<?php echo site_url("Circles/Create_Topic")?>',
    	type: 'POST',
    	//cache: false,
        ...
        progress_box.show();
  		...
  		success: function(data) {
  			if( data.status )
  	    	{ 
  	    		$(".black_feds").text('发布成功').show();
  	    		setTimeout("prompt();", 2000);
  	    		window.setTimeout("window.location.href='<?php echo site_url('Circles/?tribe_id='.$tribe_id) ?>'", 1000);   
  	        	return;
  	  	...
  	  	error:function(xhr){ //上传失败 
  	 		...
  	 		$(".black_feds").text('上传失败，请连接网络再发布话题').show();
  	 		setTimeout("prompt();", 2000);
  	 		setTimeout(function(){
    			progress_box.hide();
  	    		}, 2000);
...
// 	}
	...
	$('.introduction_xia').text('发布中....');
	$('.introduction_xia').removeAttr('onclick');
	$('.introduction_xia').css('background','#dddddd');
	$('.introduction_xia').css('color','#aaaaaa');
    ...
    }).done(function(data) 
    {
    	...
    	{ 
            ...
            //setTimeout( window.location.href= '<?php // echo site_url('Circles/Topic_Detaile')?>/'+data.id+'?tribe_id='+tribe_id, 2000);   
    		...
    		$(".black_feds").text('发布失败，请重试').show();
    		setTimeout("prompt();", 2000);
    		$('.introduction_xia').text('发布');
		    $('.introduction_xia').attr('onclick','sub()');
			$('.introduction_xia').css('background','#ffd600');
			$('.introduction_xia').css('color','#333333');
...
var JM = function(){
 ...
 //检测输入内容字数的变化
  $(".example_top").on("input propertychange",function() {
    if ($(this).val()=='') {
      $('.introduction_xia').css('background','#dddddd');
      $('.introduction_xia').css('color','#aaa');
    } else{
      $('.introduction_xia').css('background','#ffd600');
      $('.introduction_xia').css('color','#333333');
    };
